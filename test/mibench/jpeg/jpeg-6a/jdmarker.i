...

{
  ...
  if (length >= 14) {
    for (buffp = 0; buffp < 14; buffp++)
      do { if (bytes_in_buffer == 0) { if (! (*datasrc->fill_input_buffer) (cinfo)) { return 0; } ( next_input_byte = datasrc->next_input_byte, bytes_in_buffer = datasrc->bytes_in_buffer ); } bytes_in_buffer--; b[buffp] = (*next_input_byte++); } while (0);
    length -= 14;
 ...
 ((cinfo)->err->msg_code = (JTRC_JFIF_MINOR), (cinfo)->err->msg_parm.i[0] = (b[5]), (cinfo)->err->msg_parm.i[1] = (b[6]), (*(cinfo)->err->emit_message) ((j_common_ptr) (cinfo), (1)));
      ...
      do { int * _mp = (cinfo)->err->msg_parm.i; _mp[0] = (cinfo->X_density); _mp[1] = (cinfo->Y_density); _mp[2] = (cinfo->density_unit); (cinfo)->err->msg_code = (JTRC_JFIF); (*(cinfo)->err->emit_message) ((j_common_ptr) (cinfo), (1)); } while (0)
                                                                ;
      if (b[12] | b[13])
...
jinit_marker_reader (j_decompress_ptr cinfo)
{
  ...
  cinfo->marker = (struct jpeg_marker_reader *)
    ...
    ((size_t) sizeof(struct jpeg_marker_reader)));

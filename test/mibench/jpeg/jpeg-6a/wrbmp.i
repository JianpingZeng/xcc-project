...
static void write_colormap
 (j_decompress_ptr cinfo, bmp_dest_ptr dest, int map_colors, int map_entry_size)
                                          ;
...
finish_output_bmp (j_decompress_ptr cinfo, djpeg_dest_ptr dinfo)
{
  ...
  for (row = cinfo->output_height; row > 0; row--) {
    ...
    image_ptr = (*cinfo->mem->access_virt_sarray)
      ((j_common_ptr) cinfo, dest->whole_image, row-1, (JDIMENSION) 1, 0);

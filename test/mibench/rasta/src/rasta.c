...
main(int argc,char** argv)
{
  ...
  struct fvec *speech, *cepstrum, *frame;
  int nframe;
  char *funcname;
  FILE *outfp;

  if(argc == 1) /* print out usage if no arguments used */
    {
      usage(argv[0]);
      exit(0);
    }
  ...
  get_comline(&runparam, argc,argv); /* preferably list
                                        arguments in command line so there is a record */

  check_args( &runparam ); /* Exits if params out of range */

  if ((runparam.online == TRUE) || (runparam.abbotIO == TRUE))
    {
      speech = (struct fvec *)NULL;
      /* Don't try to reference this pointer ! */
    }
  else
    {
  ...
  for (nframe = 0; nframe < runparam.nframes; )
    {
      ...
      else if (runparam.abbotIO == TRUE)
        {
          ...
          if (history.eos)
            /* reset state in history structure, rasta filter, and
               noise estimation, write an eos field to the output,
               and restart at the top of the loop */
            {
              history.normInit = TRUE;
              if (runparam.history == TRUE)
                {
      ...
      write_out( &runparam, outfp, cepstrum );
    }

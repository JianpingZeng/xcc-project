          ...
          GLenum type, const GLvoid *pixels )
{
   ...
   desty = y;
   if (ctx->Depth.Test) {
      GLdepth zval = (GLdepth) (ctx->Current.RasterPos[2] * 65535.0F);
      for (i=0;i<width;i++) {
  zspan[i] = zval;
      }
   }
            ...
            GLenum type, const GLvoid *pixels )
{
  ...
  case GL_UNSIGNED_BYTE:
     {
        GLubyte *src = (GLubyte *) pixels + i * width;
        memcpy( (void *) (stencil), (void *) (src), (size_t) (width) );
     }
     break;
  case GL_BYTE:
     {
          ...
          GLenum type, const GLvoid *pixels )
{
   ...
   if (ctx->Visual->RGBAflag) {
      ...
      memset( (void *) (red), (int) (r), (size_t) (width) );
      memset( (void *) (green), (int) (g), (size_t) (width) );
      memset( (void *) (blue), (int) (b), (size_t) (width) );
      memset( (void *) (alpha), (int) (a), (size_t) (width) );
   }
   else {
      GLuint i;
      for (i=0;i<width;i++) {
  ispan[i] = ctx->Current.RasterIndex;
   ...
   if (type==GL_UNSIGNED_SHORT && sizeof(GLdepth)==sizeof(GLushort)
      ...
      for (i=0;i<height;i++,y++) {
            ...
            case GL_UNSIGNED_BYTE:
               {
                  GLubyte *src = (GLubyte *) pixels + i * width;
                  for (j=0;j<width;j++) {
                     depth[j] = ((GLfloat) (*src++) * (1.0F / 255.0F));
                  }
               }
               break;
            case GL_BYTE:
               {
                  GLbyte *src = (GLbyte *) pixels + i * width;
                  for (j=0;j<width;j++) {
                     depth[j] = ((2.0F * (*src++) + 1.0F) * (1.0F/255.0F));
                  }
               }
               break;
            case GL_UNSIGNED_SHORT:
               {
                  GLushort *src = (GLushort *) pixels + i * width;
                  for (j=0;j<width;j++) {
                     depth[j] = ((GLfloat) (*src++) * (1.0F / 65535.0F));
                  }
               }
               break;
            case GL_SHORT:
               {
                  GLshort *src = (GLshort *) pixels + i * width;
                  for (j=0;j<width;j++) {
                     depth[j] = ((2.0F * (*src++) + 1.0F) * (1.0F/65535.0F));
                  }
               }
               break;
            case GL_UNSIGNED_INT:
               {
                  GLuint *src = (GLuint *) pixels + i * width;
                  for (j=0;j<width;j++) {
                     depth[j] = ((GLfloat) (*src++) * (1.0F / 4294967295.0F));
                  }
               }
               break;
            case GL_INT:
               {
                  GLint *src = (GLint *) pixels + i * width;
                  for (j=0;j<width;j++) {
                     depth[j] = ((2.0F * (*src++) + 1.0F) * (1.0F/4294967294.0F));
                  }
               }
               break;
            case GL_FLOAT:
         ...
         for (j=0;j<width;j++) {
            zspan[j] = (GLdepth) (( (depth[j])<(0.0F) ? (0.0F) : ((depth[j])>(1.0F) ? (1.0F) : (depth[j])) ) * 65535.0F);
         }
          ...
          GLenum type, const GLvoid *pixels )
{
   ...
   if (ctx->Depth.Test) {
      ...
      GLdepth z = (GLdepth) (ctx->Current.RasterPos[2] * 65535.0F);
      for (i=0;i<width;i++) {
  zspan[i] = z;
      }
   }
   ...
   if (type==GL_UNSIGNED_BYTE && (format==GL_RGB || format == GL_LUMINANCE)
      ...
      memset( (void *) (alpha), (int) ((GLint) ctx->Visual->AlphaScale), (size_t) (width) );
      if (format == GL_RGB) {
  ...
  switch (type) {
    ...
    for (j=0;j<width;j++) {
       if (l_flag) {
   rf[j] = gf[j] = bf[j] = ((GLfloat) (*src++) * (1.0F / 255.0F));
       }
       else {
   rf[j] = r_flag ? ((GLfloat) (*src++) * (1.0F / 255.0F)) : 0.0;
   gf[j] = g_flag ? ((GLfloat) (*src++) * (1.0F / 255.0F)) : 0.0;
   bf[j] = b_flag ? ((GLfloat) (*src++) * (1.0F / 255.0F)) : 0.0;
       }
       af[j] = a_flag ? ((GLfloat) (*src++) * (1.0F / 255.0F)) : 1.0;
    }
        }
        break;
     case GL_BYTE:
    ...
    for (j=0;j<width;j++) {
       if (l_flag) {
   rf[j] = gf[j] = bf[j] = ((2.0F * (*src++) + 1.0F) * (1.0F/255.0F));
       }
       else {
   rf[j] = r_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/255.0F)) : 0.0;
   gf[j] = g_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/255.0F)) : 0.0;
   bf[j] = b_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/255.0F)) : 0.0;
       }
       af[j] = a_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/255.0F)) : 1.0;
    }
        }
        break;
     case GL_BITMAP:
    ...
    for (j=0;j<width;j++) {
       if (l_flag) {
   rf[j] = gf[j] = bf[j] = ((GLfloat) (*src++) * (1.0F / 65535.0F));
       }
       else {
   rf[j] = r_flag ? ((GLfloat) (*src++) * (1.0F / 65535.0F)) : 0.0;
   gf[j] = g_flag ? ((GLfloat) (*src++) * (1.0F / 65535.0F)) : 0.0;
   bf[j] = b_flag ? ((GLfloat) (*src++) * (1.0F / 65535.0F)) : 0.0;
       }
       af[j] = a_flag ? ((GLfloat) (*src++) * (1.0F / 65535.0F)) : 1.0;
    }
        }
        break;
     case GL_SHORT:
    ...
    for (j=0;j<width;j++) {
       if (l_flag) {
   rf[j] = gf[j] = bf[j] = ((2.0F * (*src++) + 1.0F) * (1.0F/65535.0F));
       }
       else {
   rf[j] = r_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/65535.0F)) : 0.0;
   gf[j] = g_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/65535.0F)) : 0.0;
   bf[j] = b_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/65535.0F)) : 0.0;
       }
       af[j] = a_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/65535.0F)) : 1.0;
    }
        }
        break;
     case GL_UNSIGNED_INT:
    ...
    for (j=0;j<width;j++) {
       if (l_flag) {
   rf[j] = gf[j] = bf[j] = ((GLfloat) (*src++) * (1.0F / 4294967295.0F));
       }
       else {
   rf[j] = r_flag ? ((GLfloat) (*src++) * (1.0F / 4294967295.0F)) : 0.0;
   gf[j] = g_flag ? ((GLfloat) (*src++) * (1.0F / 4294967295.0F)) : 0.0;
   bf[j] = b_flag ? ((GLfloat) (*src++) * (1.0F / 4294967295.0F)) : 0.0;
       }
       af[j] = a_flag ? af[j] = ((GLfloat) (*src++) * (1.0F / 4294967295.0F)) : 1.0;
    }
        }
        break;
     case GL_INT:
    ...
    for (j=0;j<width;j++) {
       if (l_flag) {
   rf[j] = gf[j] = bf[j] = ((2.0F * (*src++) + 1.0F) * (1.0F/4294967294.0F));
       }
       else {
   rf[j] = r_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/4294967294.0F)) : 0.0;
   gf[j] = g_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/4294967294.0F)) : 0.0;
   bf[j] = b_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/4294967294.0F)) : 0.0;
       }
       af[j] = a_flag ? ((2.0F * (*src++) + 1.0F) * (1.0F/4294967294.0F)) : 1.0;
    }
        }
        break;
     case GL_FLOAT:
  ...
  if (scale_or_bias) {
     for (j=0;j<width;j++) {
        ...
        rf[j] = ( (r)<(0.0) ? (0.0) : ((r)>(1.0) ? (1.0) : (r)) );
        gf[j] = ( (g)<(0.0) ? (0.0) : ((g)>(1.0) ? (1.0) : (g)) );
        bf[j] = ( (b)<(0.0) ? (0.0) : ((b)>(1.0) ? (1.0) : (b)) );
        af[j] = ( (a)<(0.0) ? (0.0) : ((a)>(1.0) ? (1.0) : (a)) );
     }
  }
                        ...
                        GLenum format, GLenum type, const GLvoid *pixels )
{
  ...
  default:
     gl_error( ctx, GL_INVALID_ENUM, "glDrawPixels" );
      ...
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_DRAW_PIXEL_TOKEN); } ctx->Feedback.Count++;;

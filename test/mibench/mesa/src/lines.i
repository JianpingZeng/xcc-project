...
void gl_LineStipple( GLcontext *ctx, GLint factor, GLushort pattern )
{
   ...
   ctx->Line.StippleFactor = ( (factor)<(1) ? (1) : ((factor)>(256) ? (256) : (factor)) );
...
static void feedback_line( GLcontext *ctx, GLuint v1, GLuint v2, GLuint pv )
{
   ...
   if (ctx->StippleCounter==0) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_LINE_RESET_TOKEN); } ctx->Feedback.Count++;;
   }
   else {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_LINE_TOKEN); } ctx->Feedback.Count++;;
   }
   if (ctx->Light.ShadeModel==GL_FLAT) {
      ...
      color[0] = ((VB->Color[v1][0]) * 0.00048828125f) * invRedScale;
      color[1] = ((VB->Color[v1][1]) * 0.00048828125f) * invGreenScale;
      color[2] = ((VB->Color[v1][2]) * 0.00048828125f) * invBlueScale;
      color[3] = ((VB->Color[v1][3]) * 0.00048828125f) * invAlphaScale;
      gl_feedback_vertex( ctx, x1,y1,z1,w1, color,
                          (GLfloat) VB->Index[v1], tex1 );

      color[0] = ((VB->Color[v2][0]) * 0.00048828125f) * invRedScale;
      color[1] = ((VB->Color[v2][1]) * 0.00048828125f) * invGreenScale;
      color[2] = ((VB->Color[v2][2]) * 0.00048828125f) * invBlueScale;
      color[3] = ((VB->Color[v2][3]) * 0.00048828125f) * invAlphaScale;
      gl_feedback_vertex( ctx, x2,y2,z2,w2, color,
...
static void flat_ci_line( GLcontext *ctx, GLuint v1, GLuint v2, GLuint pv )
{
   ...
   if ((PB)->index!=(VB->Index[pv]) || !(PB)->mono) { gl_flush_pb( ctx ); } (PB)->index = VB->Index[pv]; (PB)->mono = GL_TRUE;;
   ...
   if (ctx->Depth.Test) {
      ...
      { GLint zz0, zz1, dzz, ii; switch (n) { case 1: zptr[0] = z1; break; case 2: zptr[0] = z1; zptr[1] = z2; break; case 3: zptr[0] = z1; zptr[1] = ((z1)+(z2)) >> 1; zptr[2] = z2; break; default: zz0 = (z1) << 7; zz1 = (z2) << 7; dzz = (zz1-zz0) / ((n)-1); for (ii=0;ii<(n);ii++) { zptr[ii] = zz0 >> 7; zz0 += dzz; } } };
   }

   PB->count += n;
   if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
}
...
static void flat_rgba_line( GLcontext *ctx, GLuint v1, GLuint v2, GLuint pv )
{
   ...
   if ((PB)->color[0]!=(VB->Color[pv][0]) || (PB)->color[1]!=(VB->Color[pv][1]) || (PB)->color[2]!=(VB->Color[pv][2]) || (PB)->color[3]!=(VB->Color[pv][3]) || !(PB)->mono) { gl_flush_pb( ctx ); } (PB)->color[0] = VB->Color[pv][0]; (PB)->color[1] = VB->Color[pv][1]; (PB)->color[2] = VB->Color[pv][2]; (PB)->color[3] = VB->Color[pv][3]; (PB)->mono = GL_TRUE;
                                                     ;
...
static void general_ci_line( GLcontext *ctx, GLuint v1, GLuint v2, GLuint pv )
{
  ...
  mask[i] = 1;
   ...
   if (ctx->Depth.Test) {
      ...
      GLint z1 = (GLint) (VB->Win[v1][2] + ctx->LineZoffset);
      GLint z2 = (GLint) (VB->Win[v2][2] + ctx->LineZoffset);
      { GLint zz0, zz1, dzz, ii; switch (n) { case 1: z[0] = z1; break; case 2: z[0] = z1; z[1] = z2; break; case 3: z[0] = z1; z[1] = ((z1)+(z2)) >> 1; z[2] = z2; break; default: zz0 = (z1) << 7; zz1 = (z2) << 7; dzz = (zz1-zz0) / ((n)-1); for (ii=0;ii<(n);ii++) { z[ii] = zz0 >> 7; zz0 += dzz; } } };
   }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]-1; (PB)->z[(PB)->count] = z[i]; (PB)->i[(PB)->count] = index[i]; (PB)->count++;;
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->i[(PB)->count] = index[i]; (PB)->count++;;
     }
  }
      }
      else {
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        (PB)->x[(PB)->count] = x[i]-1; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->i[(PB)->count] = index[i]; (PB)->count++;;
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->i[(PB)->count] = index[i]; (PB)->count++;;
     }
  }
      }
      ...
      GLint width, w0, w1;
      width = (GLint) ( (ctx->Line.Width)<(1.0) ? (1.0) : ((ctx->Line.Width)>(10.0) ? (10.0) : (ctx->Line.Width)) );
      w0 = -width / 2;
      w1 = w0 + width - 1;
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        ...
        GLint y1 = y[i] + w1;
        for (yy=y0;yy<=y1;yy++) {
    (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = yy; (PB)->z[(PB)->count] = z[i]; (PB)->i[(PB)->count] = index[i]; (PB)->count++;;
        }
        if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
     }
  }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        ...
        GLint x1 = x[i] + w1;
        for (xx=x0;xx<=x1;xx++) {
    (PB)->x[(PB)->count] = xx; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->i[(PB)->count] = index[i]; (PB)->count++;;
        }
        if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
     }
  }
                               ...
                               GLuint v1, GLuint v2, GLuint pv )
{
   ...
   GLdepth z[1280];
   GLubyte mask[1280];
   GLubyte red[1280], green[1280], blue[1280], alpha[1280];
   GLint i, n;
   GLint dx, dy;
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]-1; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->count++;
                                             ;
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->count++;
                                             ;
     }
  }
      }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        (PB)->x[(PB)->count] = x[i]-1; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->count++;
                                             ;
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->count++;
                                             ;
     }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        ...
        GLint y1 = y[i] + w1;
        for (yy=y0;yy<=y1;yy++) {
    (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = yy; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->count++;
                                                ;
        }
        if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
     }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        ...
        GLint x1 = x[i] + w1;
        for (xx=x0;xx<=x1;xx++) {
    (PB)->x[(PB)->count] = xx; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->count++;
                                                ;
        }
        if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
     }
                                ...
                                GLuint v1, GLuint v2, GLuint pv )
{
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]-1; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->s[(PB)->count] = s[i]; (PB)->t[(PB)->count] = t[i]; (PB)->u[(PB)->count] = u[i]; (PB)->count++;
                                                            ;
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->s[(PB)->count] = s[i]; (PB)->t[(PB)->count] = t[i]; (PB)->u[(PB)->count] = u[i]; (PB)->count++;
                                                            ;
     }
  }
      }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        (PB)->x[(PB)->count] = x[i]-1; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->s[(PB)->count] = s[i]; (PB)->t[(PB)->count] = t[i]; (PB)->u[(PB)->count] = u[i]; (PB)->count++;
                                                            ;
        (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->s[(PB)->count] = s[i]; (PB)->t[(PB)->count] = t[i]; (PB)->u[(PB)->count] = u[i]; (PB)->count++;
                                                            ;
     }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        ...
        GLint y1 = y[i] + w1;
        for (yy=y0;yy<=y1;yy++) {
    (PB)->x[(PB)->count] = x[i]; (PB)->y[(PB)->count] = yy; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->s[(PB)->count] = s[i]; (PB)->t[(PB)->count] = t[i]; (PB)->u[(PB)->count] = u[i]; (PB)->count++;
                                                               ;
        }
        if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
     }
  ...
  for (i=0;i<n;i++) {
     if (mask[i]) {
        ...
        GLint x1 = x[i] + w1;
        for (xx=x0;xx<=x1;xx++) {
    (PB)->x[(PB)->count] = xx; (PB)->y[(PB)->count] = y[i]; (PB)->z[(PB)->count] = z[i]; (PB)->r[(PB)->count] = red[i]; (PB)->g[(PB)->count] = green[i]; (PB)->b[(PB)->count] = blue[i]; (PB)->a[(PB)->count] = alpha[i]; (PB)->s[(PB)->count] = s[i]; (PB)->t[(PB)->count] = t[i]; (PB)->u[(PB)->count] = u[i]; (PB)->count++;
                                                               ;
        }
        if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); };
     }

...
void gl_StencilFunc( GLcontext *ctx, GLenum func, GLint ref, GLuint mask )
{
   ...
   maxref = (1 << 8) - 1;
   ctx->Stencil.Ref = ( (ref)<(0) ? (0) : ((ref)>(maxref) ? (maxref) : (ref)) );
       ...
       GLubyte mask[] )
{
   if (ctx->Depth.Test==GL_FALSE) {
      ...
      GLubyte passmask[1280], failmask[1280], oldmask[1280];
      GLuint i;
            ...
            GLenum oper, GLubyte mask[] )
{
  ...
  if (invmask==0) {
     for (i=0;i<n;i++) {
        if (mask[i]) {
                  GLstencil *sptr = (ctx->Buffer->Stencil + ctx->Buffer->Width * (y[i]) + (x[i]));
                  *sptr = 0;
        }
     }
  }
         ...
         const GLdepth z[], GLubyte mask[] )
{
   if (ctx->Depth.Test==GL_FALSE) {
      ...
      GLubyte passmask[(3*1280)], failmask[(3*1280)], oldmask[(3*1280)];
      GLuint i;
                           ...
                           GLuint n, GLint x, GLint y, GLubyte stencil[] )
{
   ...
   if (ctx->Buffer->Stencil) {
      ...
      memcpy( (void *) (stencil), (void *) (s), (size_t) (n * sizeof(GLubyte)) );
   }
}
# 936 "stencil.c"
       ...
       const GLubyte stencil[] )
{
   ...
   if (ctx->Buffer->Stencil) {
      s = (ctx->Buffer->Stencil + ctx->Buffer->Width * (y) + (x));
      memcpy( (void *) (s), (void *) (stencil), (size_t) (n * sizeof(GLubyte)) );
   }
}
...
void gl_clear_stencil_buffer( GLcontext *ctx )
{
   ...
   if (ctx->Scissor.Enabled) {
      ...
      for (y=ctx->Buffer->Ymin; y<=ctx->Buffer->Ymax; y++) {
         GLstencil *ptr = (ctx->Buffer->Stencil + ctx->Buffer->Width * (y) + (ctx->Buffer->Xmin));
         memset( (void *) (ptr), (int) (ctx->Stencil.Clear), (size_t) (width * sizeof(GLstencil)) );
      }
   }
   else {

      memset( (void *) (ctx->Buffer->Stencil), (int) (ctx->Stencil.Clear), (size_t) (ctx->Buffer->Width * ctx->Buffer->Height * sizeof(GLstencil)) )
                                                                            ;
   }
}

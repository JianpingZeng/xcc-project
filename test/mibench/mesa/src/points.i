   ...
   for (i=first;i<=last;i++) {
      if (VB->Unclipped[i]) {
         ...
         if (ctx->Light.ShadeModel==GL_SMOOTH) {
            ...
            color[0] = ((VB->Color[i][0]) * 0.00048828125f) * invRedScale;
            color[1] = ((VB->Color[i][1]) * 0.00048828125f) * invGreenScale;
            color[2] = ((VB->Color[i][2]) * 0.00048828125f) * invBlueScale;
            color[3] = ((VB->Color[i][3]) * 0.00048828125f) * invAlphaScale;
         }
         else {
         ...
         if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_POINT_TOKEN); } ctx->Feedback.Count++;;
...
static void size1_rgba_points( GLcontext *ctx, GLuint first, GLuint last )
{
   ...
   for (i=first;i<=last;i++) {
      if (VB->Unclipped[i]) {
         ...
         (PB)->x[(PB)->count] = x; (PB)->y[(PB)->count] = y; (PB)->z[(PB)->count] = z; (PB)->r[(PB)->count] = red; (PB)->g[(PB)->count] = green; (PB)->b[(PB)->count] = blue; (PB)->a[(PB)->count] = alpha; (PB)->count++;;
      }
   }
   if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); }
}
...
static void general_ci_points( GLcontext *ctx, GLuint first, GLuint last )
{
   ...
   isize = (GLint) (( (ctx->Point.Size)<(1.0) ? (1.0) : ((ctx->Point.Size)>(10.0) ? (10.0) : (ctx->Point.Size)) ) + 0.5F);
   ...
   for (i=first;i<=last;i++) {
      if (VB->Unclipped[i]) {
         ...
         if ((PB)->index!=(VB->Index[i]) || !(PB)->mono) { gl_flush_pb( ctx ); } (PB)->index = VB->Index[i]; (PB)->mono = GL_TRUE;;

         for (iy=y0;iy<=y1;iy++) {
            for (ix=x0;ix<=x1;ix++) {
               (PB)->x[(PB)->count] = ix; (PB)->y[(PB)->count] = iy; (PB)->z[(PB)->count] = z; (PB)->count++;;
            }
         }
         if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); }
      }
...
static void general_rgba_points( GLcontext *ctx, GLuint first, GLuint last )
{
   ...
   for (i=first;i<=last;i++) {
      if (VB->Unclipped[i]) {
         ...
         if ((PB)->color[0]!=(VB->Color[i][0] >> shift) || (PB)->color[1]!=(VB->Color[i][1] >> shift) || (PB)->color[2]!=(VB->Color[i][2] >> shift) || (PB)->color[3]!=(VB->Color[i][3] >> shift) || !(PB)->mono) { gl_flush_pb( ctx ); } (PB)->color[0] = VB->Color[i][0] >> shift; (PB)->color[1] = VB->Color[i][1] >> shift; (PB)->color[2] = VB->Color[i][2] >> shift; (PB)->color[3] = VB->Color[i][3] >> shift; (PB)->mono = GL_TRUE;
...
static void textured_rgba_points( GLcontext *ctx, GLuint first, GLuint last )
{
   ...
   for (i=first;i<=last;i++) {
      if (VB->Unclipped[i]) {
         ...
         isize = (GLint)
                   (( (ctx->Point.Size)<(1.0) ? (1.0) : ((ctx->Point.Size)>(10.0) ? (10.0) : (ctx->Point.Size)) ) + 0.5F);
         if (isize<1) {
            isize = 1;
         }
         ...
         for (iy=y0;iy<=y1;iy++) {
            for (ix=x0;ix<=x1;ix++) {
               (PB)->x[(PB)->count] = ix; (PB)->y[(PB)->count] = iy; (PB)->z[(PB)->count] = z; (PB)->r[(PB)->count] = red; (PB)->g[(PB)->count] = green; (PB)->b[(PB)->count] = blue; (PB)->a[(PB)->count] = alpha; (PB)->s[(PB)->count] = s; (PB)->t[(PB)->count] = t; (PB)->u[(PB)->count] = u; (PB)->count++;;
            }
         }
         if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); }
      }
                                     ...
                                     GLuint first, GLuint last )
{
   ...
   radius = ( (ctx->Point.Size)<(1.0) ? (1.0) : ((ctx->Point.Size)>(10.0) ? (10.0) : (ctx->Point.Size)) ) * 0.5F;
   rmin = radius - 0.7071F;
   rmax = radius + 0.7071F;
   rmin2 = rmin*rmin;
   rmax2 = rmax*rmax;
   ...
   if (gl_texturing_enabled(ctx)) {
      for (i=first;i<=last;i++) {
         if (VB->Unclipped[i]) {
            ...
            for (y=ymin;y<=ymax;y++) {
               for (x=xmin;x<=xmax;x++) {
                  GLfloat dx = x - VB->Win[i][0];
                  GLfloat dy = y - VB->Win[i][1];
                  ...
                  if (dist2<rmax2) {
                     ...
                     (PB)->x[(PB)->count] = x; (PB)->y[(PB)->count] = y; (PB)->z[(PB)->count] = z; (PB)->r[(PB)->count] = red; (PB)->g[(PB)->count] = green; (PB)->b[(PB)->count] = blue; (PB)->a[(PB)->count] = alpha; (PB)->s[(PB)->count] = s; (PB)->t[(PB)->count] = t; (PB)->u[(PB)->count] = u; (PB)->count++;;
                  }
               }
            }
            if ((PB)->count>=(3*1280)-1280) { gl_flush_pb( ctx ); }

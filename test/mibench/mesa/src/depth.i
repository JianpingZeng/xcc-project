...
void gl_ClearDepth( GLcontext* ctx, GLclampd depth )
{
   ...
   ctx->Depth.Clear = (GLfloat) ( (depth)<(0.0) ? (0.0) : ((depth)>(1.0) ? (1.0) : (depth)) );
}
...
# 144 "depth.c"
   ...
   n = (GLfloat) ( (nearval)<(0.0) ? (0.0) : ((nearval)>(1.0) ? (1.0) : (nearval)) );
   f = (GLfloat) ( (farval)<(0.0) ? (0.0) : ((farval)>(1.0) ? (1.0) : (farval)) );

   ctx->Viewport.Near = n;
   ctx->Viewport.Far = f;
   ctx->Viewport.Sz = 65535.0F * ((f - n) / 2.0);
   ctx->Viewport.Tz = 65535.0F * ((f - n) / 2.0 + n);
}
# 182 "depth.c"
GLuint gl_depth_test_span_generic( GLcontext* ctx,
                                   ...
                                   GLubyte mask[] )
{
   GLdepth *zptr = ((ctx)->Buffer->Depth + (ctx)->Buffer->Width * (y) + (x));
   GLubyte *m = mask;
   GLuint i;
   GLuint passed = 0;
                                   ...
                                   const GLdepth z[], GLubyte mask[] )
{
   ...
   switch (ctx->Depth.Func) {
     ...
     for (i=0; i<n; i++) {
        if (mask[i]) {
    zptr = ((ctx)->Buffer->Depth + (ctx)->Buffer->Width * (y[i]) + (x[i]));
    if (z[i] < *zptr) {
                               ...
                               GLuint n, GLint x, GLint y, GLfloat depth[] )
{
   ...
   if (ctx->Buffer->Depth) {
      zptr = ((ctx)->Buffer->Depth + (ctx)->Buffer->Width * (y) + (x));
      for (i=0;i<n;i++) {
                             ...
                             GLuint n, GLint x, GLint y, GLdepth depth[] )
{
   if (ctx->Buffer->Depth) {
      GLdepth *zptr = ((ctx)->Buffer->Depth + (ctx)->Buffer->Width * (y) + (x));
      memcpy( (void *) (depth), (void *) (zptr), (size_t) (n * sizeof(GLdepth)) );
   }
   else {
      GLuint i;
      for (i=0;i<n;i++) {
...
{
   GLdepth clear_value = (GLdepth) (ctx->Depth.Clear * 65535.0F);
   ...
   if (ctx->Scissor.Enabled) {
      ...
      if (sizeof(GLdepth)==2 && (clear_value&0xff)==(clear_value>>8)) {
         ...
         memset( (void *) (ctx->Buffer->Depth), (int) (clear_value&0xff), (size_t) (2*ctx->Buffer->Width*ctx->Buffer->Height) )
                                                          ;
      }
      else {

                                ...
                                GLint img, GLint row, GLint column )
{
   ...
   if (type==GL_BITMAP) {
      ...
      bytes_per_row = alignment
                    * ( (comp_per_pixel*pixels_per_row) % (8*alignment) == 0 ? (comp_per_pixel*pixels_per_row)/(8*alignment) : (comp_per_pixel*pixels_per_row)/(8*alignment)+1 );
  ...
  comps_per_row = alignment / bytes_per_comp
                       * ( (bytes_per_row) % (alignment) == 0 ? (bytes_per_row)/(alignment) : (bytes_per_row)/(alignment)+1 );
      }
                                    ...
                                    GLboolean interleave )
{
   ...
   if (srcType==GL_BITMAP || destType==GL_BITMAP) {
      ...
      GLubyte *buffer, *dst;
      ((srcType==GL_BITMAP) ? (void) (0) : __assert_fail ("srcType==GL_BITMAP", "image.c", 343, __PRETTY_FUNCTION__));
      ((destType==GL_BITMAP) ? (void) (0) : __assert_fail ("destType==GL_BITMAP", "image.c", 344, __PRETTY_FUNCTION__));
      ...
      GLubyte *buffer, *dst;
      GLint i, d;
      ((srcType==GL_UNSIGNED_BYTE) ? (void) (0) : __assert_fail ("srcType==GL_UNSIGNED_BYTE", "image.c", 411, __PRETTY_FUNCTION__));
            ...
            switch (srcType) {
               ...
               case GL_FLOAT:
                  if (ctx->Unpack.SwapBytes) {
                     ...
                     memcpy( (void *) (dst), (void *) (src), (size_t) (elems_per_row*sizeof(GLfloat)) );
                     dst += elems_per_row;
                  }
                  break;
               default:

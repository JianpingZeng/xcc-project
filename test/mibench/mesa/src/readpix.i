            ...
            case GL_UNSIGNED_BYTE:
               {
                  GLubyte *dst = (GLubyte *) pixels + start + j * k;
                  for (i=0;i<width;i++) {
                     *dst++ = ((GLubyte) (GLint) (((depth[i])) * 255.0F));
                  }
               }
               break;
            case GL_BYTE:
            ...
            case GL_UNSIGNED_SHORT:
               {
                  GLushort *dst = (GLushort *) pixels + start + j * k;
                  for (i=0;i<width;i++) {
                     *dst++ = ((GLushort) (GLint) ((depth[i]) * 65535.0F));
                  }
                  if (ctx->Pack.SwapBytes) {
                     gl_swap2( (GLushort *) pixels + start + j * k, width );
                  }
            ...
            case GL_SHORT:
               {
                  GLshort *dst = (GLshort *) pixels + start + j * k;
                  for (i=0;i<width;i++) {
                     *dst++ = ( (((GLint) (65535.0F * (depth[i]))) - 1) / 2 );
                  }
                  if (ctx->Pack.SwapBytes) {
                     gl_swap2( (GLushort *) pixels + start + j * k, width );
                  }
            ...
            case GL_UNSIGNED_INT:
               {
                  GLuint *dst = (GLuint *) pixels + start + j * k;
                  for (i=0;i<width;i++) {
                     *dst++ = ((GLuint) ((depth[i]) * 4294967295.0));
                  }
                  if (ctx->Pack.SwapBytes) {
                     gl_swap4( (GLuint *) pixels + start + j * k, width );
                  }
            ...
            case GL_INT:
               {
                  GLint *dst = (GLint *) pixels + start + j * k;
                  for (i=0;i<width;i++) {
                     *dst++ = ( (GLint) (2147483647.0 * (depth[i])) );
                  }
                  if (ctx->Pack.SwapBytes) {
                     gl_swap4( (GLuint *) pixels + start + j * k, width );
                  }
     ...
     GLenum type, GLvoid *pixels )
{
  ...
  case GL_UNSIGNED_BYTE:
     {
        GLubyte *dst = (GLubyte *) pixels + start + j * k;
        memcpy( (void *) (dst), (void *) (stencil), (size_t) (width) );
     }
     break;
  case GL_BYTE:
     {
     ...
     GLfloat blue[], GLfloat alpha[] )
{
   ...
   for (i=0;i<n;i++) {
      ...
      red[i] = ( (r)<(0.0F) ? (0.0F) : ((r)>(1.0F) ? (1.0F) : (r)) );
      green[i] = ( (g)<(0.0F) ? (0.0F) : ((g)>(1.0F) ? (1.0F) : (g)) );
      blue[i] = ( (b)<(0.0F) ? (0.0F) : ((b)>(1.0F) ? (1.0F) : (b)) );
      alpha[i] = ( (a)<(0.0F) ? (0.0F) : ((a)>(1.0F) ? (1.0F) : (a)) );
   }
}
          ...
          GLenum format, GLenum type, GLvoid *pixels )
{
  ...
  gl_error( ctx, GL_INVALID_ENUM, "glReadPixels(format)" );
   ...
   if (s>=a) {
      ...
      k = a/s * ( (s*n*l) % (a) == 0 ? (s*n*l)/(a) : (s*n*l)/(a)+1 );
   }
     ...
     alpha[i] = ctx->Pixel.MapItoA[ index[i] ];
      ...
      if (l_flag) {
         for (i=0;i<width;i++) {
            ...
            luminance[i] = ( (sum)<(0.0F) ? (0.0F) : ((sum)>(1.0F) ? (1.0F) : (sum)) );
         }
      }
  ...
  case GL_UNSIGNED_BYTE:
     {
        GLubyte *dst = (GLubyte *) pixels + start + j * k;
        for (i=0;i<width;i++) {
    if (r_flag) *dst++ = ((GLubyte) (GLint) (((red[i])) * 255.0F));
    if (g_flag) *dst++ = ((GLubyte) (GLint) (((green[i])) * 255.0F));
    if (b_flag) *dst++ = ((GLubyte) (GLint) (((blue[i])) * 255.0F));
    if (l_flag) *dst++ = ((GLubyte) (GLint) (((luminance[i])) * 255.0F));
    if (a_flag) *dst++ = ((GLubyte) (GLint) (((alpha[i])) * 255.0F));
        }
     }
     break;
  case GL_BYTE:
     {
        GLbyte *dst = (GLbyte *) pixels + start + j * k;
        for (i=0;i<width;i++) {
    if (r_flag) *dst++ = ( (((GLint) (255.0F * (red[i]))) - 1) / 2 );
    if (g_flag) *dst++ = ( (((GLint) (255.0F * (green[i]))) - 1) / 2 );
    if (b_flag) *dst++ = ( (((GLint) (255.0F * (blue[i]))) - 1) / 2 );
    if (l_flag) *dst++ = ( (((GLint) (255.0F * (luminance[i]))) - 1) / 2 );
    if (a_flag) *dst++ = ( (((GLint) (255.0F * (alpha[i]))) - 1) / 2 );
        }
     }
     break;
  case GL_UNSIGNED_SHORT:
     {
        GLushort *dst = (GLushort *) pixels + start + j * k;
        for (i=0;i<width;i++) {
    if (r_flag) *dst++ = ((GLushort) (GLint) ((red[i]) * 65535.0F));
    if (g_flag) *dst++ = ((GLushort) (GLint) ((green[i]) * 65535.0F));
    if (b_flag) *dst++ = ((GLushort) (GLint) ((blue[i]) * 65535.0F));
    if (l_flag) *dst++ = ((GLushort) (GLint) ((luminance[i]) * 65535.0F));
    if (a_flag) *dst++ = ((GLushort) (GLint) ((alpha[i]) * 65535.0F));
        }
     }
     if (ctx->Pack.SwapBytes) {
  ...
  case GL_SHORT:
     {
        GLshort *dst = (GLshort *) pixels + start + j * k;
        for (i=0;i<width;i++) {
    if (r_flag) *dst++ = ( (((GLint) (65535.0F * (red[i]))) - 1) / 2 );
    if (g_flag) *dst++ = ( (((GLint) (65535.0F * (green[i]))) - 1) / 2 );
    if (b_flag) *dst++ = ( (((GLint) (65535.0F * (blue[i]))) - 1) / 2 );
    if (l_flag) *dst++ = ( (((GLint) (65535.0F * (luminance[i]))) - 1) / 2 );
    if (a_flag) *dst++ = ( (((GLint) (65535.0F * (alpha[i]))) - 1) / 2 );
        }
        if (ctx->Pack.SwapBytes) {
    gl_swap2( (GLushort *) pixels + start + j * k, width*n );
        }
  ...
  case GL_UNSIGNED_INT:
     {
        GLuint *dst = (GLuint *) pixels + start + j * k;
        for (i=0;i<width;i++) {
    if (r_flag) *dst++ = ((GLuint) ((red[i]) * 4294967295.0));
    if (g_flag) *dst++ = ((GLuint) ((green[i]) * 4294967295.0));
    if (b_flag) *dst++ = ((GLuint) ((blue[i]) * 4294967295.0));
    if (l_flag) *dst++ = ((GLuint) ((luminance[i]) * 4294967295.0));
    if (a_flag) *dst++ = ((GLuint) ((alpha[i]) * 4294967295.0));
        }
        if (ctx->Pack.SwapBytes) {
  ...
  case GL_INT:
     {
        GLint *dst = (GLint *) pixels + start + j * k;
        for (i=0;i<width;i++) {
    if (r_flag) *dst++ = ( (GLint) (2147483647.0 * (red[i])) );
    if (g_flag) *dst++ = ( (GLint) (2147483647.0 * (green[i])) );
    if (b_flag) *dst++ = ( (GLint) (2147483647.0 * (blue[i])) );
    if (l_flag) *dst++ = ( (GLint) (2147483647.0 * (luminance[i])) );
    if (a_flag) *dst++ = ( (GLint) (2147483647.0 * (alpha[i])) );
        }
        if (ctx->Pack.SwapBytes) {
    gl_swap4( (GLuint *) pixels + start + j * k, width*n );
        }

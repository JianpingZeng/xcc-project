...
{
   if (ctx->RenderMode==GL_FEEDBACK || ((ctx)->Primitive!=GL_BITMAP)) {
  ...
  break;
      case GL_3D:
  ctx->Feedback.Mask = 0x01;
         ctx->Feedback.Type = type;
  break;
      case GL_3D_COLOR:
  ctx->Feedback.Mask = 0x01
                           | (ctx->Visual->RGBAflag ? 0x08 : 0x04);
         ctx->Feedback.Type = type;
  break;
...
void gl_PassThrough( GLcontext *ctx, GLfloat token )
{
   ...
   if (ctx->RenderMode==GL_FEEDBACK) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_PASS_THROUGH_TOKEN); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (token); } ctx->Feedback.Count++;;
   }
}
...
void gl_feedback_vertex( GLcontext *ctx,
    ...
    const GLfloat texcoord[4] )
{
   if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (x); } ctx->Feedback.Count++;;
   if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (y); } ctx->Feedback.Count++;;
   if (ctx->Feedback.Mask & 0x01) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (z); } ctx->Feedback.Count++;;
   }
   if (ctx->Feedback.Mask & 0x02) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (w); } ctx->Feedback.Count++;;
   }
   if (ctx->Feedback.Mask & 0x04) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (index); } ctx->Feedback.Count++;;
   }
   if (ctx->Feedback.Mask & 0x08) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (color[0]); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (color[1]); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (color[2]); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (color[3]); } ctx->Feedback.Count++;;
   }
   if (ctx->Feedback.Mask & 0X10) {
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (texcoord[0]); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (texcoord[1]); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (texcoord[2]); } ctx->Feedback.Count++;;
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = (texcoord[3]); } ctx->Feedback.Count++;;
   }
}
# 185 "feedback.c"
...
static void write_hit_record( GLcontext *ctx )
{
   ...
   ((ctx != 0) ? (void) (0) : __assert_fail ("ctx != 0", "feedback.c", 245, __PRETTY_FUNCTION__));
   zmin = (GLuint) ((GLfloat) zscale * ctx->Select.HitMinZ);
   zmax = (GLuint) ((GLfloat) zscale * ctx->Select.HitMaxZ);

   if (ctx->Select.BufferCount < ctx->Select.BufferSize) { ctx->Select.Buffer[ctx->Select.BufferCount] = (ctx->Select.NameStackDepth); } ctx->Select.BufferCount++;;
   if (ctx->Select.BufferCount < ctx->Select.BufferSize) { ctx->Select.Buffer[ctx->Select.BufferCount] = (zmin); } ctx->Select.BufferCount++;;
   if (ctx->Select.BufferCount < ctx->Select.BufferSize) { ctx->Select.Buffer[ctx->Select.BufferCount] = (zmax); } ctx->Select.BufferCount++;;
   for (i=0;i<ctx->Select.NameStackDepth;i++) {
      if (ctx->Select.BufferCount < ctx->Select.BufferSize) { ctx->Select.Buffer[ctx->Select.BufferCount] = (ctx->Select.NameStack[i]); } ctx->Select.BufferCount++;;
   }
...
void gl_LoadName( GLcontext *ctx, GLuint name )
{
   ...
   if (ctx->Select.NameStackDepth<64) {
      ctx->Select.NameStack[ctx->Select.NameStackDepth-1] = name;
   }
   else {
      ctx->Select.NameStack[64 -1] = name;
   }
}

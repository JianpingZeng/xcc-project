                    ...
                    GLclampf blue, GLclampf alpha )
{
   ...
   ctx->Color.ClearColor[0] = ( (red)<(0.0F) ? (0.0F) : ((red)>(1.0F) ? (1.0F) : (red)) );
   ctx->Color.ClearColor[1] = ( (green)<(0.0F) ? (0.0F) : ((green)>(1.0F) ? (1.0F) : (green)) );
   ctx->Color.ClearColor[2] = ( (blue)<(0.0F) ? (0.0F) : ((blue)>(1.0F) ? (1.0F) : (blue)) );
   ctx->Color.ClearColor[3] = ( (alpha)<(0.0F) ? (0.0F) : ((alpha)>(1.0F) ? (1.0F) : (alpha)) );
...
static void clear_color_buffer_with_masking( GLcontext *ctx )
{
   ...
   if (ctx->Visual->RGBAflag) {
      ...
      GLubyte mask[1280];
      GLint i, j;
      memset( (void *) (mask), (int) (1), (size_t) (width) );
      for (i=0;i<height;i++,y++) {
         for (j=0;j<width;j++) {

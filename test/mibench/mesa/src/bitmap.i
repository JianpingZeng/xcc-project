                       ...
                       const struct gl_image *bitmap )
{
   ...
   if (ctx->NewState) {
      gl_update_state(ctx);
      (PB)->count = 0; (PB)->mono = GL_FALSE; (PB)->primitive = (GL_BITMAP);;
   }
   ...
   if (ctx->Visual->RGBAflag) {
      ...
      if ((PB)->color[0]!=(r) || (PB)->color[1]!=(g) || (PB)->color[2]!=(b) || (PB)->color[3]!=(a) || !(PB)->mono) { gl_flush_pb( ctx ); } (PB)->color[0] = r; (PB)->color[1] = g; (PB)->color[2] = b; (PB)->color[3] = a; (PB)->mono = GL_TRUE;;
   }
   else {
      if ((PB)->index!=(ctx->Current.RasterIndex) || !(PB)->mono) { gl_flush_pb( ctx ); } (PB)->index = ctx->Current.RasterIndex; (PB)->mono = GL_TRUE;;
   }

   px = (GLint) ( (ctx->Current.RasterPos[0] - xorig) + 0.0F );
   py = (GLint) ( (ctx->Current.RasterPos[1] - yorig) + 0.0F );
   pz = (GLint) ( ctx->Current.RasterPos[2] * 65535.0F );
   ...
   for (by=0;by<height;by++) {
      ...
      for (bx=0;bx<width;bx++) {
         if (*ptr&bitmask) {
            (PB)->x[(PB)->count] = px+bx; (PB)->y[(PB)->count] = py+by; (PB)->z[(PB)->count] = pz; (PB)->count++;;
         }
         bitmask = bitmask >> 1;
         if (bitmask==0) {
            ptr++;
                ...
                const struct gl_image *bitmap )
{
   ...
   if (ctx->RenderMode==GL_RENDER) {
      ...
      if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_BITMAP_TOKEN); } ctx->Feedback.Count++;;

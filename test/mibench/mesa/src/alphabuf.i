      ...
      if (abuffer) {
         ...
         if (ctx->Scissor.Enabled) {
            ...
            for (j=0;j<ctx->Scissor.Height;j++) {
               GLubyte *aptr = (ctx->Buffer->Alpha + (ctx->Buffer->Ymin+j) * ctx->Buffer->Width + (ctx->Buffer->Xmin))
                                                              ;
            ...
            memset( (void *) (abuffer), (int) (aclear), (size_t) (ctx->Buffer->Width*ctx->Buffer->Height) );
         }
      }
   }
}
...
void gl_write_alpha_span( GLcontext* ctx, GLuint n, GLint x, GLint y,
                          GLubyte alpha[], GLubyte mask[] )
{
   GLubyte *aptr = (ctx->Buffer->Alpha + (y) * ctx->Buffer->Width + (x));
   GLuint i;
                            ...
                            const GLubyte alpha[], const GLubyte mask[] )
{
   ...
   if (mask) {
      for (i=0;i<n;i++) {
         if (mask[i]) {
            GLubyte *aptr = (ctx->Buffer->Alpha + (y[i]) * ctx->Buffer->Width + (x[i]));
            *aptr = alpha[i];
         }
      }
   }

                               ...
                               GLuint v0, GLuint v1, GLuint v2, GLuint pv )
{
   ...
   if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) GL_POLYGON_TOKEN); } ctx->Feedback.Count++;;
   if (ctx->Feedback.Count < ctx->Feedback.BufferSize) { ctx->Feedback.Buffer[ctx->Feedback.Count] = ((GLfloat) 3); } ctx->Feedback.Count++;;
   ...
   for (i=0;i<3;i++) {
      ...
      if (ctx->Light.ShadeModel==GL_SMOOTH) {
         ...
         color[0] = ((VB->Color[v][0]) * 0.00048828125f) * invRedScale;
         color[1] = ((VB->Color[v][1]) * 0.00048828125f) * invGreenScale;
         color[2] = ((VB->Color[v][2]) * 0.00048828125f) * invBlueScale;
         color[3] = ((VB->Color[v][3]) * 0.00048828125f) * invAlphaScale;
      }
...
# 292 "tritemp.h"
      GLuint index = VB->Index[pv]; if (!VB->MonoColor) { (*ctx->Driver.Index)( ctx, index ); }
...
# 594 "tritemp.h"
               {
                  ...
                  fz = (GLfixed)(z0 * 2048.0f + dzdx * adjx + dzdy * adjy) + 0x00000400;
...
# 755 "tritemp.h"
               { GLint i, n = right-left; GLdepth zspan[1280]; if (n>0) { for (i=0;i<n;i++) { zspan[i] = ((ffz) >> 11); ffz += fdzdx; } gl_write_monoindex_span( ctx, n, left, iy, zspan, index, GL_POLYGON ); } };
...
# 629 "tritemp.h"
               fi = (GLfixed)(VB->Index[vLower] * 2048.0f + didx * adjx
                              + didy * adjy) + 0x00000400;
               fdiOuter = ((GLfixed) ((didy + dxOuter * didx) * 2048.0f));
# 668 "tritemp.h"
            }
...
# 752 "tritemp.h"
               ...
               { GLint i, n = right-left; GLdepth zspan[1280]; GLuint index[1280]; if (n>0) { for (i=0;i<n;i++) { zspan[i] = ((ffz) >> 11); index[i] = ((ffi) >> 11); ffz += fdzdx; ffi += fdidx; } gl_write_index_span( ctx, n, left, iy, zspan, index, GL_POLYGON ); } };
...
# 258 "tritemp.h"
   {
      ...
      GLfloat dzdx, dzdy; GLfixed fdzdx;
# 292 "tritemp.h"
      if (!VB->MonoColor) { GLubyte r = VB->Color[pv][0]; GLubyte g = VB->Color[pv][1]; GLubyte b = VB->Color[pv][2]; GLubyte a = VB->Color[pv][3]; (*ctx->Driver.Color)( ctx, r, g, b, a ); }
...
# 739 "tritemp.h"
               ...
               GLint right = ((fxRightEdge) >> 11);
# 755 "tritemp.h"
               { GLint i, n = right-left; GLdepth zspan[1280]; if (n>0) { for (i=0;i<n;i++) { zspan[i] = ((ffz) >> 11); ffz += fdzdx; } gl_write_monocolor_span( ctx, n, left, iy, zspan, VB->Color[pv][0], VB->Color[pv][1], VB->Color[pv][2], VB->Color[pv][3], GL_POLYGON ); } };
...
# 295 "tritemp.h"
...

      {
         ...
         eMaj_dr = ((VB->Color[vMax][0] - VB->Color[vMin][0]) * 0.00048828125f);
         eBot_dr = ((VB->Color[vMid][0] - VB->Color[vMin][0]) * 0.00048828125f);
      ...
      }
      {
         ...
         eMaj_dg = ((VB->Color[vMax][1] - VB->Color[vMin][1]) * 0.00048828125f);
  eBot_dg = ((VB->Color[vMid][1] - VB->Color[vMin][1]) * 0.00048828125f);
      ...
      }
      {
         ...
         eMaj_db = ((VB->Color[vMax][2] - VB->Color[vMin][2]) * 0.00048828125f);
         eBot_db = ((VB->Color[vMid][2] - VB->Color[vMin][2]) * 0.00048828125f);
  ...
  dbdy = oneOverArea * (eMaj.dx * eBot_db - eMaj_db * eBot.dx);
...

      {
         ...
         eMaj_da = ((VB->Color[vMax][3] - VB->Color[vMin][3]) * 0.00048828125f);
         eBot_da = ((VB->Color[vMid][3] - VB->Color[vMin][3]) * 0.00048828125f);
...
# 739 "tritemp.h"
               ...
               { GLint i, n = right-left; GLdepth zspan[1280]; GLubyte red[1280], green[1280]; GLubyte blue[1280], alpha[1280]; if (n>0) { for (i=0;i<n;i++) { zspan[i] = ((ffz) >> 11); red[i] = ((ffr) >> 11); green[i] = ((ffg) >> 11); blue[i] = ((ffb) >> 11); alpha[i] = ((ffa) >> 11); ffz += fdzdx; ffr += fdrdx; ffg += fdgdx; ffb += fdbdx; ffa += fdadx; } gl_write_color_span( ctx, n, left, iy, zspan, red, green, blue, alpha, GL_POLYGON ); } };
...
# 292 "tritemp.h"
      GLfloat twidth = (GLfloat) ctx->Texture.Current2D->Image[0]->Width; GLfloat theight = (GLfloat) ctx->Texture.Current2D->Image[0]->Height; GLint twidth_log2 = ctx->Texture.Current2D->Image[0]->WidthLog2; GLubyte *texture = ctx->Texture.Current2D->Image[0]->Data; GLint smask = ctx->Texture.Current2D->Image[0]->Width - 1; GLint tmask = ctx->Texture.Current2D->Image[0]->Height - 1;
...
# 366 "tritemp.h"
      {
         ...
         eMaj_ds = (VB->TexCoord[vMax][0] - VB->TexCoord[vMin][0]) * twidth;
         eBot_ds = (VB->TexCoord[vMid][0] - VB->TexCoord[vMin][0]) * theight;
         dsdx = oneOverArea * (eMaj_ds * eBot.dy - eMaj.dy * eBot_ds);
         fdsdx = ((GLfixed) ((dsdx) * 2048.0f));
         dsdy = oneOverArea * (eMaj.dx * eBot_ds - eMaj_ds * eBot.dx);
      }
      {
         GLfloat eMaj_dt, eBot_dt;
         eMaj_dt = (VB->TexCoord[vMax][1] - VB->TexCoord[vMin][1]) * twidth;
         eBot_dt = (VB->TexCoord[vMid][1] - VB->TexCoord[vMin][1]) * theight;
         dtdx = oneOverArea * (eMaj_dt * eBot.dy - eMaj.dy * eBot_dt);
         fdtdx = ((GLfixed) ((dtdx) * 2048.0f));
...
# 634 "tritemp.h"
               {
                  GLfloat s0, t0;
                  s0 = VB->TexCoord[vLower][0] * twidth;
                  fs = (GLfixed)(s0 * 2048.0f + dsdx * adjx + dsdy * adjy) + 0x00000400;
                  fdsOuter = ((GLfixed) ((dsdy + dxOuter * dsdx) * 2048.0f));
                  t0 = VB->TexCoord[vLower][1] * theight;
                  ft = (GLfixed)(t0 * 2048.0f + dtdx * adjx + dtdy * adjy) + 0x00000400;
                  fdtOuter = ((GLfixed) ((dtdy + dxOuter * dtdx) * 2048.0f));
               }
# 668 "tritemp.h"
            }
...
# 731 "tritemp.h"
               ...
               GLint right = ((fxRightEdge) >> 11);
# 755 "tritemp.h"
               { GLint i, n = right-left; GLubyte red[1280], green[1280]; GLubyte blue[1280], alpha[1280]; if (n>0) { for (i=0;i<n;i++) { GLint s = ((ffs) >> 11) & smask; GLint t = ((fft) >> 11) & tmask; GLint pos = (t << twidth_log2) + s; pos = pos + pos + pos; red[i] = texture[pos]; green[i] = texture[pos+1]; blue[i] = texture[pos+2]; alpha[i] = 255; ffs += fdsdx; fft += fdtdx; } (*ctx->Driver.WriteColorSpan)( ctx, n, left, iy, red, green, blue, alpha, ((void *)0) ); } };
...
# 731 "tritemp.h"
               ...
               GLint right = ((fxRightEdge) >> 11);
# 755 "tritemp.h"
               { GLint i, n = right-left; GLubyte red[1280], green[1280]; GLubyte blue[1280], alpha[1280]; GLubyte mask[1280]; if (n>0) { for (i=0;i<n;i++) { GLdepth z = ((ffz) >> 11); if (z < zRow[i]) { GLint s = ((ffs) >> 11) & smask; GLint t = ((fft) >> 11) & tmask; GLint pos = (t << twidth_log2) + s; pos = pos + pos + pos; red[i] = texture[pos]; green[i] = texture[pos+1]; blue[i] = texture[pos+2]; alpha[i] = 255; zRow[i] = z; mask[i] = 1; } else { mask[i] = 0; } ffz += fdzdx; ffs += fdsdx; fft += fdtdx; } (*ctx->Driver.WriteColorSpan)( ctx, n, left, iy, red, green, blue, alpha, mask ); } };
...
# 279 "tritemp.h"
      ...
      GLboolean flat_shade = (ctx->Light.ShadeModel==GL_FLAT); GLint r, g, b, a; if (flat_shade) { r = VB->Color[pv][0]; g = VB->Color[pv][1]; b = VB->Color[pv][2]; a = VB->Color[pv][3]; }
...
# 645 "tritemp.h"
    ...
    dwOuter = dwdy + dxOuter * dwdx;
                  ...
                  vLeft = v0 + (dvdx * adjx + dvdy * adjy) * (1.0F/2048.0f);
                               ...
                               GLfloat width, GLfloat height )
{
   ...
   r1 = ( (dudx)>(dudy) ? (dudx) : (dudy) );
   r2 = ( (dvdx)>(dvdy) ? (dvdx) : (dvdy) );
   rho = ( (r1)>(r2) ? (r1) : (r2) );
...
# 279 "tritemp.h"
      ...
      GLboolean flat_shade = (ctx->Light.ShadeModel==GL_FLAT); GLint r, g, b, a; GLfloat twidth, theight; if (ctx->Texture.Enabled & 4) { twidth = (GLfloat) ctx->Texture.Current3D->Image[0]->Width; theight = (GLfloat) ctx->Texture.Current3D->Image[0]->Height; } else if (ctx->Texture.Enabled & 2) { twidth = (GLfloat) ctx->Texture.Current2D->Image[0]->Width; theight = (GLfloat) ctx->Texture.Current2D->Image[0]->Height; } else { twidth = (GLfloat) ctx->Texture.Current1D->Image[0]->Width; theight = 1.0; } if (flat_shade) { r = VB->Color[pv][0]; g = VB->Color[pv][1]; b = VB->Color[pv][2]; a = VB->Color[pv][3]; }
...
# 734 "tritemp.h"
               ...
               { GLint i, n = right-left; GLdepth zspan[1280]; GLubyte red[1280], green[1280]; GLubyte blue[1280], alpha[1280]; GLfloat s[1280], t[1280], u[1280]; GLfloat lambda[1280]; if (n>0) { if (flat_shade) { for (i=0;i<n;i++) { GLdouble wwvvInv = 1.0 / (ww*vv); zspan[i] = ((ffz) >> 11); red[i] = r; green[i] = g; blue[i] = b; alpha[i] = a; s[i] = ss*wwvvInv; t[i] = tt*wwvvInv; u[i] = uu*wwvvInv; lambda[i] = compute_lambda( s[i], t[i], dsdx, dsdy, dtdx, dtdy, ww, twidth, theight ); ffz += fdzdx; ss += dsdx; tt += dtdx; uu += dudx; vv += dvdx; ww += dwdx; } } else { for (i=0;i<n;i++) { GLdouble wwvvInv = 1.0 / (ww*vv); zspan[i] = ((ffz) >> 11); red[i] = ((ffr) >> 11); green[i] = ((ffg) >> 11); blue[i] = ((ffb) >> 11); alpha[i] = ((ffa) >> 11); s[i] = ss*wwvvInv; t[i] = tt*wwvvInv; u[i] = uu*wwvvInv; lambda[i] = compute_lambda( s[i], t[i], dsdx, dsdy, dtdx, dtdy, ww, twidth, theight ); ffz += fdzdx; ffr += fdrdx; ffg += fdgdx; ffb += fdbdx; ffa += fdadx; ss += dsdx; tt += dtdx; uu += dtdx; vv += dvdx; ww += dwdx; } } gl_write_texture_span( ctx, n, left, iy, zspan, s, t, u, lambda, red, green, blue, alpha, GL_POLYGON ); } };
   ...
   if (ctx->RenderMode==GL_RENDER) {
      ...
      if (ctx->Driver.TriangleFunc) {
         ...
         if ( (ctx->Texture.Enabled==2)
             ...
             && ctx->IdentityTexMat
             && (ctx->RasterMask==0x004 || ctx->RasterMask==0)

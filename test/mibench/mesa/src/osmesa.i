...
# 67 "osmesa.c" 2
# 1 "../include/GL/osmesa.h" 1
# 80 "../include/GL/osmesa.h"
# 1 "../include/GL/gl.h" 1
# 94 "../include/GL/gl.h"
typedef enum {
...
# 81 "../include/GL/osmesa.h" 2
# 119 "../include/GL/osmesa.h"
typedef struct osmesa_context *OSMesaContext;
# 137 "../include/GL/osmesa.h"
extern OSMesaContext OSMesaCreateContext( GLenum format,
                                          OSMesaContext sharelist );
# 148 "../include/GL/osmesa.h"
extern void OSMesaDestroyContext( OSMesaContext ctx );
# 179 "../include/GL/osmesa.h"
...
static void compute_row_addresses( OSMesaContext ctx )
{
   ...
   if (ctx->yup) {
      ...
      if (ctx->format==GL_COLOR_INDEX) {
         ...
         if ((ctx->format==GL_RGB) || (ctx->format==0x4)) {
                             ...
                             GLsizei width, GLsizei height )
{
   if (!ctx || !buffer || type!=GL_UNSIGNED_BYTE
       || width<1 || height<1 || width>1280 || height>1024) {
      return GL_FALSE;
   }
...
{
   OSMesaContext osmesa = (OSMesaContext) ctx->DriverCtx;
   if (osmesa->format==GL_COLOR_INDEX) {
      if (all) {
   ...
   else if ((osmesa->format==GL_RGB)||(osmesa->format==0x4)) {
                                 ...
                                 GLuint v0, GLuint v1, GLuint pv )
{
   ...
   unsigned long pixel = ( ((VB->Color[pv][0]) << osmesa->rshift) | ((VB->Color[pv][1]) << osmesa->gshift) | ((VB->Color[pv][2]) << osmesa->bshift) | ((0) << osmesa->ashift) )
                                                        ;
   { GLint w = ctx->Buffer->Width; if ((x1==w) | (x2==w)) { if ((x1==w) & (x2==w)) return; x1 -= (x1==w); x2 -= (x2==w); } };
   { GLint h = ctx->Buffer->Height; if ((y1==h) | (y2==h)) { if ((y1==h) & (y2==h)) return; y1 -= (y1==h); y2 -= (y2==h); } };




   { GLint dx, dy, xf, yf, ta, tb, tt, i; if (x1!=x2 || y1!=y2) { if (x2>x1) { dx = x2-x1; xf = 1; } else { dx = x1-x2; xf = -1; } if (y2>y1) { dy = y2-y1; yf = 1; } else { dy = y1-y2; yf = -1; } if (dx>dy) { ta = dy+dy; tt = ta-dx; tb = tt-dx; for (i=0;i<=dx;i++) { { GLuint *ptr4 = ((GLuint *) osmesa->rowaddr[y1] + (x1)); *ptr4 = pixel; } x1 += xf; if (tt<0) { tt += ta; } else { tt += tb; y1 += yf; } } } else { ta = dx+dx; tt = ta-dy; tb = tt-dy; for (i=0;i<=dy;i++) { { GLuint *ptr4 = ((GLuint *) osmesa->rowaddr[y1] + (x1)); *ptr4 = pixel; } y1 += yf; if (tt<0) { tt += ta; } else { tt += tb; x1 += xf; } } } } };
                                   ...
                                   GLuint v0, GLuint v1, GLuint pv )
{
   ...
   { GLint dx, dy, xstep, ystep, ta, tb, tt, i; GLint dz, dzdx, dzdy; GLdepth *zptr; if (x1!=x2 || y1!=y2) { z1 = z1 << 7; z2 = z2 << 7; if (x2>x1) { dx = x2-x1; xstep = 1; dzdx = 1; } else { dx = x1-x2; xstep = -1; dzdx = -1; } if (y2>y1) { dy = y2-y1; ystep = 1; dzdy = ctx->Buffer->Width; } else { dy = y1-y2; ystep = -1; dzdy = -ctx->Buffer->Width; } zptr = ((ctx)->Buffer->Depth + (ctx)->Buffer->Width * (y1) + (x1)); if (dx>dy) { dz = (z2-z1)/dx; ta = dy+dy; tt = ta-dx; tb = tt-dx; for (i=0;i<=dx;i++) { GLdepth z = z1>>7; if (z < *zptr) { { GLuint *ptr4 = ((GLuint *) osmesa->rowaddr[y1] + (x1)); *ptr4 = pixel; } *zptr = z; } x1 += xstep; zptr += dzdx; if (tt<0) { tt += ta; } else { tt += tb; y1 += ystep; zptr += dzdy; } z1 += dz; } } else { dz = (z2-z1)/dy; ta = dx+dx; tt = ta-dy; tb = tt-dy; for (i=0;i<=dy;i++) { GLdepth z = z1>>7; if (z < *zptr) { { GLuint *ptr4 = ((GLuint *) osmesa->rowaddr[y1] + (x1)); *ptr4 = pixel; } *zptr = z; } y1 += ystep; zptr += dzdy; if (tt<0) { tt += ta; } else { tt += tb; x1 += xstep; zptr += dzdx; } z1 += dz; } } } };
...
static line_func choose_line_function( GLcontext *ctx )
{
   ...
   if (ctx->Light.ShadeModel!=GL_FLAT) return ((void *)0);
...
static triangle_func choose_triangle_function( GLcontext *ctx )
{
   ...
   if ((osmesa->format==GL_RGB)||(osmesa->format==0x4)) return ((void *)0);
...
static void osmesa_setup_DD_pointers( GLcontext *ctx )
{
   ...
   if ((osmesa->format==GL_RGB)||(osmesa->format==0x4)) {

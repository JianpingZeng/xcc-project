...
unsigned char *frame[];
{
  ...
  chrom_hsize = (chroma_format==3) ? horizontal_size
                                           : horizontal_size>>1;
  chrom_vsize = (chroma_format!=1) ? vertical_size
                                           : vertical_size>>1;
...
unsigned char *frame[];
{
  ...
  _IO_getc (fd); _IO_getc (fd);
  pbm_getint(fd); pbm_getint(fd); pbm_getint(fd);
  ...
  for (i=0; i<vertical_size; i++)
  {
    ...
    {
      r=_IO_getc (fd); g=_IO_getc (fd); b=_IO_getc (fd);

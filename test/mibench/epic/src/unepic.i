...
# 23 "/home/xlous/Development/experiment/arm-2010.09/bin/../arm-none-linux-gnueabi/libc/usr/include/sys/time.h" 3 4
...
# 24 "/home/xlous/Development/experiment/arm-2010.09/bin/../arm-none-linux-gnueabi/libc/usr/include/sys/time.h" 2 3 4
...
# 26 "/home/xlous/Development/experiment/arm-2010.09/bin/../arm-none-linux-gnueabi/libc/usr/include/sys/time.h" 2 3 4
...
main(argc, argv)
  ...
  while ( im_count < (3*num_levels+1) )
      {
      fread(&temp_byte,1,1,infile); the_tag=temp_byte;
   ...
   fread(&temp_byte,1,1,infile); temp_short = temp_byte; temp_short <<=8; fread(&temp_byte,1,1,infile); temp_short |= temp_byte; bin_size[index]=temp_short;
   im_count++;
   fread(&temp_byte,1,1,infile); the_tag=temp_byte;
   }
      ...
      if ( the_tag == '\000' )
   {
   fread((q_pyr+block_pos), 1, sizeof(*(q_pyr+block_pos)) * block_size, infile);
   printf("Read %ld bytes of Raw data.\n", (ftell(infile)-temp));
   temp = ftell(infile);
   }
      else if ( the_tag == '\001' )
   {
   fread(&temp_byte,1,1,infile); temp_int = temp_byte; temp_int <<= 8; fread(&temp_byte,1,1,infile); temp_int |= temp_byte; temp_int <<= 8; fread(&temp_byte,1,1,infile); temp_int |= temp_byte; temp_int <<= 8; fread(&temp_byte,1,1,infile); temp_int |= temp_byte; symbol_stream_length = temp_int;;
  ...
  if (1) write_pgm_image(outfile, result, x_size, y_size);

...
zero(filt,x_dim,y_dim,x_pos,y_pos,result,f_or_e)
  ...
  for (y_filt=0, y_res=y_start;
      ...
      if ((y_res >= 0) && (y_res < filt_sz))
 for (x_filt=y_filt, x_res=x_start;
      x_filt<y_filt+x_dim;
      x_filt++, x_res++)
   if ((x_res >= 0) && (x_res < x_dim))
...
reflect2(filt,x_dim,y_dim,x_pos,y_pos,result,f_or_e)
  ...
  for (y_filt=0, y_edge=y_edge_dist;
      ...
      {
      if (y_edge == 0) y_edge+=x_dim;
      for (x_filt=y_filt, x_edge=x_edge_dist;
    x_filt<y_filt+x_dim;
    x_filt++, x_edge++)
   {
   if (x_edge == 0) x_edge++;
   result[(y_base-(y_edge>=0 ? y_edge : -(y_edge))+x_dim>=0 ? y_base-(y_edge>=0 ? y_edge : -(y_edge))+x_dim : -(y_base-(y_edge>=0 ? y_edge : -(y_edge))+x_dim)) + (x_base-(x_edge>=0 ? x_edge : -(x_edge))+1>=0 ? x_base-(x_edge>=0 ? x_edge : -(x_edge))+1 : -(x_base-(x_edge>=0 ? x_edge : -(x_edge))+1))]
     += filt[x_filt];
   }
      }
  }
...
reflect1(filt,x_dim,y_dim,x_pos,y_pos,result,f_or_e)
  ...
  if (f_or_e == 1)
      ...
      else if (x_pos == -mx_pos) { x_start = x_dim/2; x_edge_dist = 0; }

      if (y_pos == my_pos) y_stop = x_dim*((y_dim+1)/2);
      else if (y_pos == -my_pos) { y_start = x_dim*(y_dim/2); y_edge_dist = 0;}
      }
  ...
  x_filt++, x_edge++)
      result[(y_base-(y_edge>=0 ? y_edge : -(y_edge))>=0 ? y_base-(y_edge>=0 ? y_edge : -(y_edge)) : -(y_base-(y_edge>=0 ? y_edge : -(y_edge)))) + (x_base-(x_edge>=0 ? x_edge : -(x_edge))>=0 ? x_base-(x_edge>=0 ? x_edge : -(x_edge)) : -(x_base-(x_edge>=0 ? x_edge : -(x_edge))))]
 += filt[x_filt];


  if (f_or_e == 1)
      {
      if ( ((x_pos>=0 ? x_pos : -(x_pos)) != mx_pos) && (x_pos != 0) )
 for (y_filt=x_base; y_filt<filt_sz; y_filt+=x_dim)
   result[y_filt] += result[y_filt];
      if ( ((y_pos>=0 ? y_pos : -(y_pos)) != my_pos) && (y_pos != 0) )
...
extend(filt,x_dim,y_dim,x_pos,y_pos,result,f_or_e)
  ...
  x_filt<y_filt+x_stop;
  x_filt++, x_edge++)
      if (((!y_base && (( ((y_edge)>0)?1:(((y_edge)<0)?-1:0) ) == -1))
    ||
    (y_base && (( ((y_edge)>0)?1:(((y_edge)<0)?-1:0) ) == 1)))
   !=
   ((!x_base && (( ((x_edge)>0)?1:(((x_edge)<0)?-1:0) ) == -1))
    ||
    (x_base && (( ((x_edge)>0)?1:(((x_edge)<0)?-1:0) ) == 1))))
   {
   result[(y_base-(y_edge>=0 ? y_edge : -(y_edge))>=0 ? y_base-(y_edge>=0 ? y_edge : -(y_edge)) : -(y_base-(y_edge>=0 ? y_edge : -(y_edge)))) + (x_base-(x_edge>=0 ? x_edge : -(x_edge))>=0 ? x_base-(x_edge>=0 ? x_edge : -(x_edge)) : -(x_base-(x_edge>=0 ? x_edge : -(x_edge))))]
     -= filt[x_filt];
   result[( ((y_base+y_edge)<(0))?(0):(((y_base+y_edge)>=(y_dim))?(y_dim-1):(y_base+y_edge)) ) + ( ((x_base+x_edge)<(0))?(0):(((x_base+x_edge)>=(x_dim))?(x_dim-1):(x_base+x_edge)) )]
     += filt[x_filt] + filt[x_filt];
   }
      else result[(y_base-(y_edge>=0 ? y_edge : -(y_edge))>=0 ? y_base-(y_edge>=0 ? y_edge : -(y_edge)) : -(y_base-(y_edge>=0 ? y_edge : -(y_edge)))) + (x_base-(x_edge>=0 ? x_edge : -(x_edge))>=0 ? x_base-(x_edge>=0 ? x_edge : -(x_edge)) : -(x_base-(x_edge>=0 ? x_edge : -(x_edge))))]
   += filt[x_filt];
...
nocompute(filt,x_dim,y_dim,x_pos,y_pos,result,f_or_e)
  ...
  if ( (x_pos>1) || (x_pos<-1) || (y_pos>1) || (y_pos<-1) )
...
ereflect(filt,x_dim,y_dim,x_pos,y_pos,result,f_or_e)
 ...
 += filt[x_filt];
  ...
  if (x_pos != 0)
    for (y_filt=x_base; y_filt<filt_sz; y_filt+=x_dim)
      result[y_filt] *= 1.4142135623730951;
  if (y_pos != 0)
    for (x_filt=y_base; x_filt<y_base+x_dim; x_filt++)
      result[x_filt] *= 1.4142135623730951;

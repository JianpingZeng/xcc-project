...
collapse_pyr(pyr, result, x_size, y_size, num_levels)
  ...
  register int *im;
  register int rxsize, rysize,
               ixsize, iysize,
               ipos;
  register int ccount, lcount;
  register int rx2size;
  int level;
 ...
 for (ccount = 1; ccount < ixsize; ipos++, rpos+=2, ccount++)
  ...
  result[row_rpos+1] += val;
   ...
   result[rxsize*(rysize-1)+1] += val>>1;
      ...
      im = pyr+(2*ixsize*iysize);
      internal_int_transpose(im, ixsize, iysize);
 ...
 for (ccount = 1; ccount < ixsize; ipos++, rpos+=2, ccount++)
  ...
  result[row_rpos+1] -= val;
   ...
   if (val = im[ipos])
      ...
      for (ipos=ixsize*(iysize-1), rpos=rxsize*(rysize-1)+1, ccount=1;
    ccount<ixsize; ipos++, rpos+=2, ccount++)
   {
   if (val = (im[ipos] >> 1))
  {
 ...
 for (ccount = 1; ccount < ixsize; ipos++, rpos+=2, ccount++)
  ...
  result[row_rpos+1] -= val;
   ...
   if (val = - (im[ipos] >> 1))
      ...
      for (ipos=0, rpos=rxsize, ccount=1;
    ccount<iysize; ipos+=ixsize, rpos+=rx2size, ccount++)
   {
   if (val = im[ipos])
       {
   ...
   if (val = im[ipos])
      ...
      for (ipos=ixsize-1, rpos=rx2size-1, ccount=1;
    ccount<iysize; ipos+=ixsize, rpos+=rx2size, ccount++)
   {
   if (val = (im[ipos] >> 1))
       {

...
# 63 "huffman.c"
int huffman_encode (symbol_stream, symbol_stream_length,
      packed_huffman_tree, huffman_tree_size, encoded_stream)
  SymbolType *symbol_stream;
  int symbol_stream_length;
  Byte **packed_huffman_tree;
  int *huffman_tree_size;
  CodeType *encoded_stream;
  {
  struct node *histogram[65536];
  struct node *huffman_tree;
  int encoded_stream_length;
  register int i;


  for (i=0; i<65536; i++) histogram[i] = (struct node *) ((void *)0);
...
compute_histogram(symbol_stream, symbol_stream_length, histogram)
  register SymbolType *symbol_stream;
  register int symbol_stream_length;
  register struct node *histogram[];
  {
  register int i;
  ...
  for (i=0; i<symbol_stream_length; i++, symbol_stream++)
   ...
   the_leaf->zero_child = the_leaf->one_child = ((void *)0);
   }
      else
   (the_leaf->frequency)++;
      }
...
build_huffman_tree(histogram, tree_ptr)
  register struct node *histogram[];
  struct node **tree_ptr;
  {
  struct node *tree = ((void *)0);
  register struct node *new_node;
  char temp_code[ 65536 ];
  register int i;


  for (i=0; i< 65536; i++)
    if (histogram[i] != ((void *)0))
 insert_in_ordered_list(histogram[i], &tree);




  while (tree->next != ((void *)0))
      {
...
insert_in_ordered_list(the_node, list)
  register struct node *the_node, **list;
  {
  register struct node *the_list;

  the_list = *list;

  if ( (the_list == ((void *)0)) || (the_node->frequency <= the_list->frequency) )
      {
...
compute_code_strings(tree, code_string, code_string_pos)
  register struct node *tree;
  register char code_string[];
  register int code_string_pos;
  {

  if ( tree->zero_child == ((void *)0) )
      {
...
struct code_node *read_huffman_tree(file)
  FILE *file;
  {
  ...
  fread(&temp_byte,1,1,file); the_byte=temp_byte;
  ...
  if ( the_byte == 0 )
      ...
      {
      the_node->zero_child = the_node->one_child = ((void *)0);
      the_symbol = 0;
...
read_and_huffman_decode(file, tree, symbol_stream, count)
  FILE *file;
  struct code_node *tree;
  SymbolType *symbol_stream;
  int count;
  {
  CodeType the_byte;
  ...
  for (i=0; i<count; i++)
      {
      the_node = tree;
      while (the_node->zero_child != ((void *)0))
   {
   if (bit_mask == 0) { fread(&temp_byte,1,1,file); the_byte=temp_byte; bit_mask = 1; }
...
int encode_stream(symbol_stream, symbol_stream_length, histogram, encoded_stream)
  SymbolType *symbol_stream;
  int symbol_stream_length;
  struct node *histogram[];
  CodeType *encoded_stream;
  {
  int i;
...
int pack_huffman_tree(tree, packed_tree)
  struct node *tree;
  Byte **packed_tree;
  {
  ...
  return (packed_tree_ptr - *packed_tree);
  }
# 303 "huffman.c"
pack_tree_iter(tree, packed_tree_ptr)
  struct node *tree;
...
int count_leaves (tree)
  struct node *tree;
  {
  if (tree->zero_child == ((void *)0)) return(1);

...
internal_filter(image, x_dim, y_dim, filt, temp, x_fdim, y_fdim,
  ...
  for (y_pos=ygrid_start-y_fmid-1,res_pos=0;
       y_pos<0;
       y_pos+=ygrid_step)
      {
   ...
   {
   (*reflect)(filt,x_fdim,y_fdim,x_pos,y_pos,temp,0);
   sum=0.0;
   ...
   result[res_pos] = sum;
      ...
      first_col = x_pos+1;
      (*reflect)(filt,x_fdim,y_fdim,0,y_pos,temp,0);
      for (x_pos=first_col;
    x_pos<last_ctr_col;
    x_pos+=xgrid_step,res_pos++)
   {
   ...
   result[res_pos] = sum;
      ...
      rt_edge_res_pos = res_pos + x_res_dim;
  ...
  for (x_pos=xgrid_start-x_fmid-1;
      ...
      (*reflect)(filt,x_fdim,y_fdim,x_pos,0,temp,0);
      for (y_pos=first_row;
    y_pos<last_ctr_row;
  ...
  (*reflect)(filt,x_fdim,y_fdim,0,0,temp,0);
  for (y_pos=first_row;
       y_pos<last_ctr_row;
...
internal_expand(image,filt,temp,x_fdim,y_fdim,xgrid_start,xgrid_step,
  ...
  for (y_pos=ygrid_start-y_fmid-1,im_pos=0;
       y_pos<0;
       y_pos+=ygrid_step)
      {
      for (x_pos=xgrid_start-x_fmid-1;
   ...
   {
   (*reflect)(filt,x_fdim,y_fdim,x_pos,y_pos,temp,1);
   val = image[im_pos];
   ...
   x_filt++,res_pos++)
      ...
      first_col = x_pos+1;
      (*reflect)(filt,x_fdim,y_fdim,0,y_pos,temp,1);
      for (x_pos=first_col;
    x_pos<last_ctr_col;
    x_pos+=xgrid_step,im_pos++)
   {
  ...
  for (x_pos=xgrid_start-x_fmid-1;
      ...
      {
      im_pos = prev_im_pos;
      (*reflect)(filt,x_fdim,y_fdim,x_pos,0,temp,1);
      for (y_pos=first_row;
    y_pos<last_ctr_row;
  ...
  (*reflect)(filt,x_fdim,y_fdim,0,0,temp,1);
  for (y_pos=first_row;
       y_pos<last_ctr_row;
       y_pos+=ygrid_step_full)
      {

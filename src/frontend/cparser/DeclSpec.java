...
import static frontend.cparser.DeclSpec.ParsedSpecifiers.*;
import static frontend.cparser.DeclSpec.SCS.*;
import static frontend.cparser.DeclSpec.TQ.*;
import static frontend.cparser.DeclSpec.TSC.*;
import static frontend.cparser.DeclSpec.TSW.*;
import static frontend.cparser.DeclSpec.TSS.*;
import static frontend.cparser.DeclSpec.TST.*;

/**
 * This class captures information about "declaration specifiers",
 * which encompasses storage-class-specifiers, frontend.type-specifiers,
 * frontend.type-qualifiers, and function-specifiers.
 *
 * @author Xlous.zeng
 * @version 0.1
 */
public class DeclSpec
{
    ...
    public static class DeclaratorChunk<T>
    {
        ...
        ChunkKind kind;
        /**
         * The place where this frontend.type was defined.
         */
        int loc;
        /**
        ...
        {
            // the frontend.type qualifiers : const/volatile/rstrict
            int typeQuals;
            ...
            OutParamWrapper<String> diag)
    {
        assert !isDeclRep(val) && !isTypeRep(val)
                :"rep required for these frontend.type-spec kinds!";
        if (typeSpecType  != TST_unspecified)
        {
    ...
    {
        assert isTypeRep(val):"T does not store a frontend.type";
        assert ty != null:"no frontend.type provided!";
    ...
    public void finish(Parser p)
    {
        ...
        // Validate the width of the frontend.type.
        ...
        if (typeSpecComplex != TSC_unspecified)
        {
            if (typeSpecType != TST_unspecified)
            {
                p.syntaxError(TSCLoc, "plain '_Complex' requires a frontend.type specifier; assuming '_Complex double'");
        ...
        // Okay, now we can infer the real frontend.type.
        // 'data definition has no frontend.type or storage class'?
    }
    ...
    {
        assert isDeclRep(typeSpecType) :"DeclSpec does not stores a frontend.type";
        return typeRep;
    }

...
import frontend.type.ConstantArrayType;
...
import frontend.ast.Tree.CompoundStmt;
...
import frontend.ast.Tree.IfStmt;
...
import frontend.ast.Tree.UnaryExpr;
import frontend.ast.Tree.VarDef;
import frontend.exception.CompletionFailure;

/**
 * This is main context-dependent analysis phase in driver internal. It
 * encompasses frontend.type checking and constant folding as sub task. Some subtask
...
public class Attr extends StmtVisitor implements SymbolKinds, TypeClass, Flags
{
	...
	/**
	 * Current expected proto-frontend.type
	 */
	private Type pt;
	 ...
	 * frontend.symbol. Reporting completion failure error at given position.
	 *
	 * @param pos   The position encoded to be error report.
	 * @param sym   The method frontend.symbol.
	 */
	...
	public void attriMethod(MethodSymbol sym)
	{
		...
		// check.checkNonCyclic(Position.NOPOS, sym.frontend.type);
	...
	/**
	 * Attribute a frontend.type tree.
	 *
	 * @param tree
	 * @return
	 */
	...
	/**
	 * Attributes a tree, catching possible completion failure frontend.exception. ReturnInst
	 * the tree's frontend.type.
	 *
	 * @param tree      The tree to be visited
	 * @param env       The environment visitor argument.
	 * @param protoKind The proto-kind of visitor argument.
	 * @param protoType The proto-frontend.type of visitor argument.
	 * @return
	 */
	...
	/**
	 * Obtain an otherwise unused method frontend.type with given argument types. Take it
	...
	/**
	 * Checks kind and frontend.type of given tree against proto-kind and proto-frontend.type. IfStmt
	 * checks succeeds, stores frontend.type in tree and return it. Otherwise fails,
	 * store errType in tree and return it.
	 * <p>
	 * No checks performed if the proto-frontend.type is a method frontend.type. It is not
	 * neccesary in this case since we know that kind and frontend.type are correct.
	 *
	 * @param tree       The tree whose kind and frontend.type is checked
	 * @param owntype    The computed frontend.type of this tree.
	 * @param ownkind    The computed kind of this tree.
	 * @param expectKind The expected kind of this tree.
	 * @param expectType The expected frontend.type of this tree.
	 * @return
	 */
			...
			Type expectType)
	{
		if (owntype.tag != Error && expectType.tag != Function)
		{
			...
			{
				log.error(tree.pos, "unexpected.frontend.type",
	...
	/**
	 * Determine frontend.type of identifier or select expression and check that (1) the
	 * referenced frontend.symbol is not deprecated (2) the frontend.symbol's frontend.type is safe (@see
	 * checkSafe) (3) if frontend.symbol is a variable, check that its frontend.type and kind are
	 * compatible with the prototype and protokind. (4) if frontend.symbol is an instance
	 * field of a raw frontend.type, which is being assigned to, issue an unchecked
	 * warning if its frontend.type changes under erasure. (5) if frontend.symbol is an instance
	 * method of a raw frontend.type, issue an unchecked warning if its argument types
	 * change under erasure. IfStmt checks succeed: IfStmt frontend.symbol is a constant, return
	 * its constant frontend.type else if frontend.symbol is a method, return its getReturnValue frontend.type
	 * otherwise return its frontend.type. Otherwise return errType.
	 *
	 * @param tree  The syntax tree representing the identifier
	 * @param sym   The frontend.symbol representing the identifier.
	 * @param env   The current environment.
	 * @param pkind The set of expected kinds.
	 * @param pt    The expected frontend.type.
	 */
	 ...
	 * @param env  The current environment.
	 * @param v    The variable's frontend.symbol.
	 */
	...
	/**
	 * Can the given frontend.symbol be the owner of code which forms part if class
	 * initialization? This is the case if the frontend.symbol is a frontend.type or field, or if
	 * the frontend.symbol is the synthetic method. owning a block.
	 */
	...
	/**
	 * Compute the frontend.type of a conditional expression, after
	 * checking that it exists. See Spec 15.25.
	 *
	 * @param pos      The source position to be used for error diagnostics.
	 * @param condtype The frontend.type of the expression's condition.
	 * @param thentype The frontend.type of the expression's then-part.
	 * @param elsetype The frontend.type of the expression's else-part.
	 */
	...
	/**
	 * ReturnInst the frontend.type of a literal with given frontend.type tag.
	 */
	...
	@Override public void visitNewArray(NewArray tree)
	{
		...
		else
		{
			...
			else
			{
				if (pt.tag != Error)
				{
					log.error(tree.pos, "illegal.initializer.for.frontend.type",
							pt.toString());
				}

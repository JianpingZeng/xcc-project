...
import static frontend.ast.CastKind.CK_Invalid;
import static frontend.ast.Tree.ExprValueKind.EVK_RValue;
import static frontend.sema.BinaryOperatorKind.*;
import static frontend.sema.UnaryOperatorKind.*;
import static frontend.type.Type.IntTy;
import static frontend.type.Type.UnsignedIntTy;
 ...
 * tree's frontend.type and position. No other fields are kept in a tree node; instead
...
{
	/*
	 * TopLevel nodes, of frontend.type TopLevel, representing entire source files.
	 */
	...
	/**
	 * Function definitions, of frontend.type MethodDef.
	 */
	public static final int MethodDefStmtClass = SelectExprClass + 1;

	/**
	 * GlobalVariable definitions, of frontend.type VarDef.
	 */
	public static final int VarDefStmtClass = MethodDefStmtClass + 1;

	/**
	 * The no-op statement ";", of frontend.type NullStmt.
	 */
	...
	/**
	 * Expression statements, of frontend.type Exec.
	 */
	public static final int ExprStmtClass = IfStmtClass + 1;

	/**
	 * BreakStmt statements, of frontend.type BreakStmt.
	 */
	...
	/**
	 * Function invocation expressions, of frontend.type CallExpr.
	 */
	public static final int CallExprClass = ReturnStmtClass + 1;

	/**
	 * Parenthesized subexpressions of frontend.type ParenExpr.
	 */
	...
	/**
	 * Implicit frontend.type cast expressions.
	 */
	...
	/**
	 * BinaryExpr operators, of frontend.type BinaryExpr.
	 */
	...
	/**
	 * Error Trees, of frontend.type ErroneousTree.
	 */
	...
	public static class SelectStmt extends Stmt
	{
		...
		public Name name;
		/**
		 * frontend.symbol of the selected class
		 */
		public Symbol sym;
	...
	public abstract static class Expr extends Stmt
    {
        ...
        public QualType isPromotableBitField()
        {
            ...
            // to the getTypeSize of int, it gets promoted no matter what its frontend.type is.
        ...
        public long getIntWidth()
        {
            ...
            if (type.getType().isBooleanType())
                return 1;
            // for the primitive frontend.type, just use the standard frontend.type size.
        ...
        public boolean isConstantInitializer()
        {
            switch (tag)
            {
                ...
                case InitListExprClass:
                {
                    ...
                    // to bitfields.
                    frontend.ast.Tree.InitListExpr expr = (Tree.InitListExpr)this;
    ...
    public static class IntegerLiteral extends Expr
    {
                ...
                int loc)
        {
            ...
            assert type.isIntegerType():"Illegal frontend.type in Integer literal.";
            assert value.getBitWidth() == type.getTypeSize()
                    :"Integer frontend.type is not the correct size for constant.";
            val = value;
        }
    ...
    /**
     * Allows us to explicitly represent implicit frontend.type conversions, which have
    ...
    public static final class ImplicitCastExpr extends CastExpr
    {
        ...
        /**
         * ReturnStmt the expression after ignoring all of implicitly frontend.type cast operation.
         * @return
         */
    ...
    public static class CompoundLiteralExpr extends Expr
    {
        ...
        /**
         * This can be an incomplete array frontend.type, in
         * which case the actual expression frontend.type will be different.
         */
        private QualType ty;
        private Expr init;
    ...
    {
        /**
         * true if operand is a frontend.type, false if it is an expression.
         */
     ...
     * track of the frontend.type the operation is performed in.  Due to the semantics of
     * these operators, the operands are promoted, the arithmetic performed, an
     * implicit conversion back to the result frontend.type done, then the assignment takes
     * place.  This captures the intermediate frontend.type which the computation is done
     * in.
     */
    public static class CompoundAssignExpr extends BinaryExpr
    {
        ...
        /**
         * The two computation types are the frontend.type the LHS is converted
         * to for the computation and the frontend.type of the result; the two are
     ...
     * placeholder frontend.type "void". This initializer list is called the

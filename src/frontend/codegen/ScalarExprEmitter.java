...
import frontend.ast.Tree.BinaryExpr;
import frontend.ast.Tree.CompoundAssignExpr;
...
public class ScalarExprEmitter extends StmtVisitor<Value>
{
    ...
    private Value emitAdd(BinOpInfo info)
    {
        ...
        if (info.ty.isPointerType() &&
                info.ty.<frontend.type.PointerType>getAs().isVariableArrayType())
        {
        ...
        Value ptr, idx;
        Expr idxExpr;
        frontend.type.PointerType pt = info.expr.getLHS().getType().<frontend.type.PointerType>getAs();
        if (pt != null)
        {
            ptr = info.lhs;
            idx = info.rhs;
        ...
        {
            pt = info.expr.getRHS().getType().<frontend.type.PointerType>getAs();
    ...
    private Value visitPrePostIncDec(UnaryExpr expr, boolean isInc, boolean isPrec)
    {
        ...
        if (valTy.isPointerType() && valTy.<frontend.type.PointerType>getAs().isVariableArrayType())
        {

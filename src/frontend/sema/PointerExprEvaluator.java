...
public class PointerExprEvaluator extends ExprEvaluatorBase<Boolean>
{
    ...
    public Boolean visitBinaryExpr(Tree.BinaryExpr expr)
    {
        ...
        // let the expr of frontend.type pointer frontend.type reside at left side
        // (pointer) +- (rhs).
        if (iExpr.getType().isPointerType())
            Util.swap(pExpr, iExpr);

        // If the pExpr is not pointer frontend.type, return false.
        if (!evaluatePointer(pExpr, result))
            return false;

        // So that, the pExpr must be pointer frontend.type

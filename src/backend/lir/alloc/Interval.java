...
import backend.lir.ci.LIRAddress;
import backend.lir.ci.LIRKind;
import backend.lir.ci.LIRRegisterValue;
...
public final class Interval
{
	...
	/**
	 * list of use positions. Each entry in the list records the use position and
	 * register priority associated with the use position. The entries in the list
	 * are in descending order of use position.
	 */
	public static final class UsePosList
	{
		...
		/**
		 * Splits this list around a given position. All entries in this list with
		 * a use position greater or equal than {@code splitPos} are removed from
		 * this list and added to the returned list.
		 *
		 * @param splitPos the position for the split
		 * @return a use position list containing all entries removed from this
		 * list that have a use position greater or equal than {@code splitPos}
		 */
		 ...
		 *
		 * @param index the index of the entry for which the use position is
		 *              returned
		...
		/**
		 * Gets the register priority for the use position at a specified index
		 * in this list.
		 *
		 * @param index the index of the entry for which the register priority
		 *              is returned
		...
		@Override public String toString()
		{
			...
			for (int i = size() - 1; i >= 0; --i)
			{
				...
				buf.append(usePos(i)).append(" -> ").append(prio.ordinal())
						.append(':').append(prio);
			}
	...
	/**
	 * The {@linkplain backend.lir.ci.LIRRegisterValue register} or
	 * {@linkplain backend.lir.ci.LIRVariable variable} for register allocation.
	 */
	 ...
	 * among {@link Interval.State#Active}, {@link Interval.State#Handled},
	 * {@link Interval.State#Unhandled} and {@link Interval.State#Inactive}.
	 */
	State state;
	...
	/**
	 * List of all intervals that are split off from this interval. This is only
	 * used if this is a {@linkplain #isSplitParent() split parent}.
	 */
	private List<Interval> splitChildren = Collections.emptyList();

	/**
	 * Current split child that has been active or inactive last (always stored
	 * in split parents).
	 */
	private Interval currentSplitChild;

	/**
	 * Specifies if move is inserted between currentSplitChild and this interval
	 * when interval gets active the first time.
	 */
	...
	/**
	 * Gets the {@linkplain LIRRegisterValue register}, {@linkplain StackSlot spill slot}
	 * or {@linkplain LIRAddress address} assigned to this interval.
	 */
	public LIRValue location()
	{
		return location;
	}
	...
	void setSpillSlot(StackSlot slot)
	{
		...
		assert parent.spillSlot
				== null : "connot overwrite existing spill slot";
	...
	{
		return splitParent().spillState == SpillState.StoreAtDefinition
				|| splitParent().spillState == SpillState.StartInMemory;
	}
	 ...
	 */
	static final Interval EndMarker = new Interval(LIRValue.IllegalValue, -1);

	Interval(LIRValue operand, int operandNumber)
	{
	...
	boolean checkSplitChildren()
	{
		if (!splitChildren.isEmpty())
		{
			...
			for (int i = 0; i < splitChildren.size(); i++)
			{
				...
				assert i1.splitParent()
						== this : "not a split child of this interval";
				assert i1.kind()
						== kind() : "must be equal for all split children";
				assert i1.spillSlot()
						== spillSlot() : "must be equal for all split children";
				...
				for (int j = i + 1; j < splitChildren.size(); j++)
				{
					...
					if (i1.from() < i2.from())
					{
						assert i1.to() <= i2.from() && i1.to() < i2
								.to() : "intervals overlapping";
					}
					else
					{
						assert i2.from() < i1
								.from() : "intervals start at same opId";
						assert i2.to() <= i1.from() && i2.to() < i1
								.to() : "intervals overlapping";
					}
				}
			}
	...
	public Interval locationHint(boolean searchSplitChild, LinearScan allocator)
	{
		...
		{
			assert locationHint
					.isSplitParent() : "ony split parents are valid hint registers";

			if (locationHint.location != null && locationHint.location
					.isRegister())
			{
				return locationHint;
			}
			else if (!locationHint.splitChildren.isEmpty())
			{
				...
				for (int i = 0; i < len; i++)
				{
					...
					if (interval.location != null && interval.location
							.isRegister())
					{
						return interval;
					}
	...
	Interval getSplitChildAtOpId(int opId, LIRInstruction.OperandMode mode,
			LinearScan allocator)
	{
		assert isSplitParent() : "can only be called for split parents";
		assert opId
				>= 0 : "invalid opId (method cannot be called for spill moves)";
		...
		else
		{
			...
			for (i = 0; i < len; i++)
			{
				...
				if (cur.from() <= opId && opId < cur.to() + toOffset)
				{
					if (i > 0)
					{
						// exchange current split child to start of list
						// (faster access for next call)
						Util.atPutGrow(splitChildren, i, splitChildren.get(0),
								null);
	...
	private boolean checkSplitChild(Interval result, int opId,
			LinearScan allocator, int toOffset, LIRInstruction.OperandMode mode)
	{
		if (result == null)
		{
			// this is an error
			StringBuilder msg = new StringBuilder(this.toString())
					.append(" has no child at ").append(opId);
			if (!splitChildren.isEmpty())
			{
				Interval first = splitChildren.get(0);
				Interval last = splitChildren.get(splitChildren.size() - 1);
				msg.append(" (first = ").append(first).append(", last = ")
						.append(last).append(")");
			}
		...
		if (!splitChildren.isEmpty())
		{
			...
			{
				if (interval != result && interval.from() <= opId
	...
	boolean splitChildCovers(int opId, LIRInstruction.OperandMode mode)
	{
		assert isSplitParent() : "can only be called for split parents";
		assert opId
				>= 0 : "invalid opId (method can not be called for spill moves)";
	...
	{
		assert operand
				.isVariable() : "cannot access use positions for fixed intervals";
	...
	int nextUsage(RegisterPriority minRegisterPriority, int from)
	{
		...
		for (int i = usePosList.size() - 1; i >= 0; --i)
		{
			...
			if (usePos >= from && usePosList.registerPriority(i)
					.greaterEqual(minRegisterPriority))
			{
				return usePos;
			}
		}
	...
	int nextUsageExact(RegisterPriority exactRegisterPriority, int from)
	{
		...
		for (int i = usePosList.size() - 1; i >= 0; --i)
		{
			int usePos = usePosList.usePos(i);
			if (usePos >= from
					&& usePosList.registerPriority(i) == exactRegisterPriority)
			{
				return usePos;
			}
		}
	...
	int previousUsage(RegisterPriority minRegisterPriority, int from)
	{
		...
		for (int i = usePosList.size() - 1; i >= 0; --i)
		{
			...
			if (usePosList.registerPriority(i)
					.greaterEqual(minRegisterPriority))
			{
				prev = usePos;
			}
	...
	{
		assert covers(pos,
				LIRInstruction.OperandMode.Input) : "use position not covered by live range";
		...
		if (registerPriority != RegisterPriority.None && operand.isVariable())
		{
			...
			if (C1XOptions.DetailedAsserts)
			{
				for (int i = 0; i < usePosList.getTypeSize(); i++)
				{
					...
					if (i > 0)
					{
						assert usePosList.usePos(i) < usePosList
								.usePos(i - 1) : "not sorted descending";
					}
				}
			}*/
			...
			else if (usePosList.registerPriority(len - 1)
					.lessThan(registerPriority))
			{
				assert usePosList.usePos(len - 1)
						== pos : "list not sorted correctly";
	...
	void addRange(int from, int to)
	{
		...
		assert first() == Range.EndMarker || to
				< first().next.from : "not inserting at begin of interval";
	...
	{
		assert operand.isVariable() : "cannot split fixed intervals";
		assert splitPos > from()
				&& splitPos < to() : "can only split inside interval";
		assert splitPos > first.from
				&& splitPos <= first.to : "can only split inside first range";
		assert firstUsage(RegisterPriority.None)
				> splitPos : "can not split when use positions are present";
	...
	@Override public String toString()
	{
		...
		return operandNumber + ":" + operand + (operand.isRegister() ?
				"" :
				location) + "[" + from + "," + to + "]";
	}
	...
	public String logString(LinearScan allocator)
	{
		...
		for (int i = usePosList.size() - 1; i >= 0; --i)
		{
			...
			buf.append(usePosList.usePos(i)).append(':')
					.append(usePosList.registerPriority(i));
			prev = usePosList.usePos(i);
		}
		return buf.append("} spill-state{").append(spillState()).append("}")
				.toString();
	}
}

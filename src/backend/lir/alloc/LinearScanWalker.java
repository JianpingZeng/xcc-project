...
import static backend.lir.LIRInstruction.OperandMode.Input;
import static backend.lir.LIRInstruction.OperandMode.Output;
import static backend.lir.ci.LIRRegister.RegisterFlag.Byte;
import static backend.lir.ci.LIRRegister.RegisterFlag.*;
 ...
 *     </ol>
			...
			int maxSplitPos)
	{
		...
		if (optimalSplitPos == interval.to()
				&& interval.nextUsage(Interval.RegisterPriority.MustHaveRegister, minSplitPos)
				== Integer.MAX_VALUE)
		{
		...
		unhandledLists.addToListSortedByStartAndUsePositions(
				Interval.RegisterBinding.Any, splitPart);
	 ...
	 * been assigned to {@linkplain backend.lir.ci.StackSlot StackSlot}
	 * @param interval
	 */
	private void splitStackInterval(Interval interval)
	{
		...
		int maxSplitPos = Math
				.min(interval.firstUsage(
						Interval.RegisterPriority.ShouldHaveRegister), interval.to());
	...
	private void combineSpilledIntervals(Interval interval)
	{
		...
		if (interval.spillState() != Interval.SpillState.NoOptimization
		...
		assert interval.firstUsage(Interval.RegisterPriority.MustHaveRegister)
				== beginPos : "must have use position at begin of interval because of move";
		assert endHint.firstUsage(Interval.RegisterPriority.MustHaveRegister)
		...
		if (backend.machineInfo.arch.isX86())
		{
			...
			if (isOdd(pos))
			{
				...
				if (pos < allocator.maxOpId() && allocator.hasCall(pos + 1)
						&& interval.to() > pos + 1 &&
						interval.nextUsage(
								Interval.RegisterPriority.ShouldHaveRegister, pos) > pos + 1)
				{
	...
	{
		Interval interval = inactiveLists.get(Interval.RegisterBinding.Fixed);
		while (interval != Interval.EndMarker)
		{
	...
	{
		Interval interval = inactiveLists.get(Interval.RegisterBinding.Any);
		while (interval != Interval.EndMarker)
		{
	...
	void freeCollectUnhandled(Interval.RegisterBinding kind, Interval current)
	{
		...
		while (interval != Interval.EndMarker)
		{
			...
			if (kind == Interval.RegisterBinding.Fixed && current.to() <= interval.from())
			{
	...
	{
		Interval interval = activeLists.get(Interval.RegisterBinding.Fixed);
		while (interval != Interval.EndMarker)
		{
			excludeFromUse(interval);
			interval = interval.next;
	...
	{
		Interval interval = unhandledLists.get(Interval.RegisterBinding.Fixed);
		while (interval != Interval.EndMarker)
		{
	...
	{
		Interval interval = activeLists.get(Interval.RegisterBinding.Any);
		while (interval != Interval.EndMarker)
		{
			setUsePos(interval,
					Math.min(interval.nextUsage(
							Interval.RegisterPriority.LiveAtLoopEnd, currentPosition),
							interval.to()), false);
			interval = interval.next;
		}
	}
 ...
 *         <li>IfStmt the location of interval at the end of predecessor and begin of successor
	...
	{
		int minSplitPos = Math.max(interval.previousUsage(
				Interval.RegisterPriority.ShouldHaveRegister,
	...
	private boolean allocFreeRegister(Interval interval)
	{
		...
		assert unhandledLists.get(Interval.RegisterBinding.Fixed) == Interval.EndMarker :
	...
	void splitForSpilling(Interval interval)
	{
		...
		int minSplitPos = Math
				.max(interval.previousUsage(
						Interval.RegisterPriority.ShouldHaveRegister, maxSplitPos)
						+ 1, interval.from());
		...
		assert interval.state
				== Interval.State.Active : "why spill interval that is not active?";
	...
	{
		assert interval.state == Interval.State.Active
				|| interval.state == Interval.State.Inactive : "other states not allowed";
		...
		else
		{
			...
			int maxSplitPos = Math
					.min(interval.nextUsage(
							Interval.RegisterPriority.MustHaveRegister, minSplitPos),
							interval.to());

			splitBeforeUsage(interval, minSplitPos, maxSplitPos);

			assert interval.nextUsage(Interval.RegisterPriority.MustHaveRegister, currentPos)
	...
	void allocateBlockedReg(Interval interval)
	{
		...
		assert unhandledLists.get(Interval.RegisterBinding.Fixed) == Interval.EndMarker : sb.toString();

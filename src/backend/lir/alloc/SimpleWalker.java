...
public class SimpleWalker
{
	...
	Backend backend;
	Interval.RegisterBindingLists unhandledList;
	Interval.RegisterBindingLists activeList;
	Interval.RegisterBindingLists inactiveList;
	 ...
	 */
	Interval.RegisterBinding currentBinding;
			...
			Interval unhandledAny)
	{
		...
		unhandledList = new Interval.RegisterBindingLists(unhandledFixed, unhandledAny);
		activeList = new Interval.RegisterBindingLists(Interval.EndMarker, Interval.EndMarker);
		inactiveList = new Interval.RegisterBindingLists(Interval.EndMarker, Interval.EndMarker);
	...
	{
		Interval.RegisterBinding binding = null;
	...
	private void walkTo(Interval.State state, int toOpID)
	{
		...
		for (Interval.RegisterBinding binding : Interval.RegisterBinding.VALUES)
		{
			...
			Interval prev = state == Interval.State.Active ? activeList.get(binding) :
			...
			while (next.from() <= toOpID)
			{
				...
				rangeHasRanged = rangeHasRanged || state == Interval.State.Inactive && cur.currentFrom() <= toOpID;
				if (rangeHasRanged)
				{
					if (prevprev == null)
					{
					...
					else if (cur.currentFrom() <= toOpID)
					{
						...
						if (prev == cur)
						{
							assert state == Interval.State.Active;
							prevprev = prev;
							prev = cur.next;
						}
					...
					else
					{
						...
						if (prev == cur)
						{
							assert state == Interval.State.Inactive;
							prevprev = prev;
							prev = cur.next;
						}

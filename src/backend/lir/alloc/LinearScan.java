...
import static tools.Util.isEven;
...
public final class LinearScan
{
	...
	final backend.lir.linearScan.OperandPool operands;
	 ...
	 * space covering all the {@linkplain backend.lir.ci.LIRVariable variables} and
	 * {@linkplain backend.lir.ci.LIRRegisterValue registers} being processed by allocator.
	 *
	 * @param opr
	 * @return
	 */
	...
	private Interval addUse(LIRValue operand, int from, int to,
			Interval.RegisterPriority priority, LIRKind kind)
	{
		...
		switch (op.opcode)
		{
			...
			case Cos:
			{
						...
						.getCallerSaveRegisters())
				{
					if (r.isFpu())
					{
						addTemp(r.asValue(), opId, Interval.RegisterPriority.None,
								LIRKind.Illegal);
					}
				}
				break;
	 ...
	 */
	private Interval.RegisterPriority registerPriorityOfOutputOperand(
	 ...
	 * @param kind
	 */
	private void addDef(LIRValue operand, int defPos, Interval.RegisterPriority priority,
			LIRKind kind)
	{
		if (!isProcessed(operand))
			return;
		...
		if (priority == Interval.RegisterPriority.None && interval.spillState().ordinal()
	 ...
	 */
	private Interval.RegisterPriority registerPriorityOfInputOperand(
			LIRInstruction instr, LIRValue operand)
	{
		if (instr.opcode == LIROpcode.Move)
		{
	...
	private void buildIntervalList()
	{
		...
		for (int i = blockCount() - 1; i >= 0; i--)
		{
					...
					.nextSetBit(operandNum + 1))
			{
				...
				addUse(operand, blockFrom, blockTo + 2, Interval.RegisterPriority.None,
						LIRKind.Illegal);
			...
			for (int j = instructions.size() - 1; j >= 1; j--)
			{
				...
				for (int k = 0; k < n; k++)
				{
					...
					addTemp(tmp, opID, Interval.RegisterPriority.MustHaveRegister,
							tmp.kind.stackKind());
				}
				...
				for (int k = 0; k < n; k++)
				{
					...
					Interval.RegisterPriority priority = registerPriorityOfInputOperand(
							opr, input);
				...
				else
				{
					...
					assert interval == Interval.EndMarker || (
							interval.isSplitParent() && interval.spillState()
									== Interval.SpillState.StoreAtDefinition) : "invalid interval";

...
public class HIRGenModule extends StmtVisitor
{
			...
			switch (expr.tag)
			{
				...
				case Tree.LITERAL:
					if (((Literal) expr).typetag == TypeClass.BOOL)
					{
						...
						log.error(expr.pos, "not.boolean.frontend.type.at.condition");
					return;
				default:
					break;
			}
	 ...
	 * Doing frontend.type cast.
	 *
	 * @param src   The source LIROperand.
	 * @param sType The source frontend.type of LIROperand
	 * @param dType The targetAbstractLayer frontend.type into that LIROperand will be casted.
	 * @return
	 */
	private Value emitCast(Value src, Type sType, Type dType)
	{
		/*
		// the original frontend.type
		int srccode = Module.typecode(sType);
		// the targetAbstractLayer frontend.type
	...
	/**
	 * Creates a temp variable with given frontend.type.
	...
	private AllocaInst createEnterBlockAlloca(LIRKind kind, VarDef var)
	{
		...
		// associate its local with variable frontend.symbol
	...
	private void emitAllocaForVarDecl(VarDef var)
	{
		...
		// setting default inst for different inst frontend.type
		else
		{
	...
	@Override public void visitSwitch(SwitchStmt tree)
	{
		...
		// At actually, the constant folding backend.opt should be taken.
	 ...
	 * @return ReturnInst null if return frontend.type is void, otherwise, return value.
	 */
	private Value emitCall(Function m, Value[] args)
	{
		...
		// return null if it's return frontend.type is void
	 ...
	 *
	 * @param ty  The data frontend.type of ret.
	...
	private Value emitADD(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the add IR.");
		}
	...
	private Value emitSUB(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the subtract IR.");
		}
		Value inst = new ArithmeticOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	private Value emitMUL(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the multiple IR.");
		}
		Value inst = new ArithmeticOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	private Value emitDIV(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the division IR.");
		}
		Value inst = new ArithmeticOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	private Value emitMOD(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the mod IR.");
		}
		Value inst = new ArithmeticOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	private Value emitBITAND(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the bitwise and IR.");
		}
	...
	private Value emitBITOR(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the bitwise or IR.");
		}
		Value inst = new LogicOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	private Value emitBITXOR(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the bitwise xor IR.");
		}
		Value inst = new LogicOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	private Value emitSHL(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the shift left IR.");
		}
	...
	private Value emitSHR(Type ty, int pos, Value lhs, Value rhs)
	{
		...
		{
			log.error(pos, "Invalid data frontend.type in the shift right IR.");
		}
		Value inst = new ShiftOp(type2Kind(ty), opcode, lhs, rhs,"");
		appendInst(inst);
		return inst;
	...
	/**
	 * Translates frontend.type cast expression.
	 *
	 * @param tree
	 */
	...
	@Override public void visitIdent(DeclRefExpr tree)
	{
		...
		// for handling aggregate frontend.type, like array and struct

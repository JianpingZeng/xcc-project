...
import jlang.symbol.Scope.Entry;
...
/**
 * A helpful utility class for resovling method getName and jlang.type.
 *
 * @author JianpingZeng
 * @version 1.0
 */
public class Resolve implements TypeClass, SymbolKinds, Flags
{
	...
	/**
	 * IfStmt `sym' is a bad jlang.symbol: report error and return errSymbol else pass
	 * through unchanged, additional arguments duplicate what has been used in
	 * trying to find the jlang.symbol (--> flyweight pattern). This improves
	 * performance since we expect misses to happen frequently.
	 *
	 * @param sym The jlang.symbol that was found, or a ResolveError.
	 * @param pos The position to use for error reporting.
	 * @param site The original jlang.type from where the selection took place.
	 * @param name The jlang.symbol's getName.
	...
	/**
	 * Find best qualified method matching given getName, jlang.type and value
	 * parameters.
	 * 
	 * @param env The current environment.
	 * @param site The original jlang.type from where the selection takes place.
	 ...
	 * @param optag The tag of the operation tree.
	 * @param env The environment current at the operation.
	 * @param argtype The jlang.type of the LIROperand.
	 */
	 ...
	 * @param optag The tag of the operation tree.
	 * @param env The environment current at the operation.
	 * @param left The jlang.type of the first LIROperand.
	 * @param right	The jlang.type of the second LIROperand.
	 */
	...
	/**
	 * Is every element of src list as same as jlang.type of one from dest list?
	...
	/**
	 * Is every element of src list a subtype of jlang.type of one from dest list?
	 * return true if succeed, otherwise return false.
	 * 
	 * @param src
	 * @param dest
    ...
    /**
     * A localized string describing the kind of a missing jlang.symbol, given an
     *  error kind.
     */
   ...
   static boolean isStatic(Env env) {
	...
	private static class ResolveError extends Symbol implements TypeClass
	{
		...
		/**
		 * The jlang.symbol that was determined by resolution, or errSymbol if none
		 * was found.
		 */
		final Symbol sym;

		/**
		 * The jlang.symbol that was a close mismatch, or null if none was found.
		 ...
		 * @param name The getName of the jlang.symbol to be resolved.
		 * @param argtypes The invocation's value parameters, if we looked for a
		 *            method.
		 */

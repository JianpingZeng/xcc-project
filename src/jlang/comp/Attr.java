 ...
 * encompasses jlang.type checking and constant folding as sub task. Some subtask
...
public class Attr extends StmtVisitor implements SymbolKinds, TypeClass, Flags
{
	...
	/**
	 * Current expected proto-jlang.type
	 */
	private Type pt;
	 ...
	 * jlang.symbol. Reporting completion failure error at given position.
	 *
	 * @param pos   The position encoded to be error report.
	 * @param sym   The method jlang.symbol.
	 */
	...
	public void attriMethod(MethodSymbol sym)
	{
		...
		// check.checkNonCyclic(Position.NOPOS, sym.jlang.type);
	...
	/**
	 * Attributes a tree, catching possible completion failure jlang.exception. ReturnInst
	 * the tree's jlang.type.
	 *
	 * @param tree      The tree to be visited
	 * @param env       The environment visitor argument.
	 * @param protoKind The proto-kind of visitor argument.
	 * @param protoType The proto-jlang.type of visitor argument.
	 * @return
	 */
	...
	/**
	 * Obtain an otherwise unused method jlang.type with given argument types. Take it
	...
	/**
	 * Checks kind and jlang.type of given tree against proto-kind and proto-jlang.type. IfStmt
	 * checks succeeds, stores jlang.type in tree and return it. Otherwise fails,
	 * store errType in tree and return it.
	 * <p>
	 * No checks performed if the proto-jlang.type is a method jlang.type. It is not
	 * neccesary in this case since we know that kind and jlang.type are correct.
	 *
	 * @param tree       The tree whose kind and jlang.type is checked
	 * @param owntype    The computed jlang.type of this tree.
	 * @param ownkind    The computed kind of this tree.
	 * @param expectKind The expected kind of this tree.
	 * @param expectType The expected jlang.type of this tree.
	 * @return
	 */
			...
			Type expectType)
	{
		if (owntype.tag != Error && expectType.tag != Function)
		{
			...
			{
				log.error(tree.pos, "unexpected.jlang.type",
	...
	/**
	 * Determine jlang.type of identifier or select expression and check that (1) the
	 * referenced jlang.symbol is not deprecated (2) the jlang.symbol's jlang.type is safe (@see
	 * checkSafe) (3) if jlang.symbol is a variable, check that its jlang.type and kind are
	 * compatible with the prototype and protokind. (4) if jlang.symbol is an instance
	 * field of a raw jlang.type, which is being assigned to, issue an unchecked
	 * warning if its jlang.type changes under erasure. (5) if jlang.symbol is an instance
	 * method of a raw jlang.type, issue an unchecked warning if its argument types
	 * change under erasure. IfStmt checks succeed: IfStmt jlang.symbol is a constant, return
	 * its constant jlang.type else if jlang.symbol is a method, return its getReturnValue jlang.type
	 * otherwise return its jlang.type. Otherwise return errType.
	 *
	 * @param tree  The syntax tree representing the identifier
	 * @param sym   The jlang.symbol representing the identifier.
	 * @param env   The current environment.
	 * @param pkind The set of expected kinds.
	 * @param pt    The expected jlang.type.
	 */
	 ...
	 * @param env  The current environment.
	 * @param v    The variable's jlang.symbol.
	 */
	...
	/**
	 * Can the given jlang.symbol be the owner of code which forms part if class
	 * initialization? This is the case if the jlang.symbol is a jlang.type or field, or if
	 * the jlang.symbol is the synthetic method. owning a block.
	 */
	...
	/**
	 * Compute the jlang.type of a conditional expression, after
	 * checking that it exists. See Spec 15.25.
	 *
	 * @param pos      The source position to be used for error diagnostics.
	 * @param condtype The jlang.type of the expression's condition.
	 * @param thentype The jlang.type of the expression's then-part.
	 * @param elsetype The jlang.type of the expression's else-part.
	 */
	...
	/**
	 * ReturnInst the jlang.type of a literal with given jlang.type tag.
	 */
	...
	@Override public void visitNewArray(NewArray tree)
	{
		...
		else
		{
			...
			else
			{
				if (pt.tag != Error)
				{
					log.error(tree.pos, "illegal.initializer.for.jlang.type",
							pt.toString());
				}

...
import frontend.ast.Pretty;
import frontend.ast.Tree;
import frontend.comp.Attr;
import frontend.comp.Enter;
import frontend.comp.Env;
import frontend.comp.Todo;
...
public final class Frontend
{
	...
	private Options backend.opt;
	public Frontend(Context context)
	{
		backend.opt = Options.instance(context);
		debugParser = backend.opt.isDebugParser();
	...
	public Tree doParseAttribute(String file)
	{
		...
		if(errorCount() == 0)
		{
			...
			// syntax fatal checking and frontend.symbol entering
			enter.main(root);
	...
	public Tree.TopLevel parse(String filename, InputStream input)
	{
		...
		if (input != null)
		{
			...
			try
			{
				Parser frontend.parser = new Parser(input, true, context);
				tree = frontend.parser.compilationUnit();
				if (debugParser)

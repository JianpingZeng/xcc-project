 ...
 * CCITT ADPCM decoder
 *
 * Usage : decode [-3|4|5] [-a|u|l] -f infile
 */
#include <stdio.h>
#include "g72x.h"
	...
	int			bits)
{
	...
	if (in_bits < bits) {
		if (fread(&in_byte, sizeof (char), 1, fp_in) != 1) {
			*code = 0;
			return (-1);
		}
...
fprintf(stderr, "\tdecode [-3|4|5] [-a|u|l] -f infile\n");
...
fprintf(stderr, "\t-l\tGenerate 16-bit linear PCM data\n");
	...
	while (unpack_input(&code, dec_bits) >= 0) {
		...
		if (out_size == 2) {
			fwrite(&sample, out_size, 1, fp_out);
		} else {
			code = (unsigned char)sample;
			fwrite(&code, out_size, 1, fp_out);
		}
	}
	fclose(fp_out);
}

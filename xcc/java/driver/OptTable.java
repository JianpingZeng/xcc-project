...
import tools.Util;
import driver.Arg.PositionalArg;
import driver.Option.OptionGroup;

import static driver.XCCFlags.RenderAsInput;
import static driver.XCCFlags.RenderJoined;
...
public class OptTable {
  ...
  public Option constructOption(int index) {
    ...
    if ((flags & XCCFlags.CC1Option) != 0)
      res.setCC1Option(true);
    if ((flags & XCCFlags.CLOption) != 0)
      res.setCLOption(true);
    if ((flags & XCCFlags.CoreOption) != 0)
      res.setCoreOption(true);
    if ((flags & XCCFlags.DriverOption) != 0)
      res.setDriverOption(true);
    if ((flags & XCCFlags.LinkerInput) != 0)
      res.setLinkerInput(true);
    if ((flags & XCCFlags.NoArgumentUnused) != 0)
      res.setNoArgumentUnused(true);
    if ((flags & XCCFlags.Unsupported) != 0)
      res.setUnsupported(true);
    if ((flags & XCCFlags.HelpHidden) != 0)
      res.setHelpHidden(true);
    if ((flags & XCCFlags.NoForward) != 0)

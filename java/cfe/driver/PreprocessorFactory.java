...
import cfe.support.PreprocessorInitOptions;
...
class PreprocessorFactory {
  ...
  private void initializePreprocessorInitOptions(
    ...
    if (!CFrontEnd.D_Macros.isEmpty()) {
      for (String m : CFrontEnd.D_Macros)
        initOpts.addMacroDef(m);
    }

    if (!CFrontEnd.U_macros.isEmpty()) {
      for (String u : CFrontEnd.U_macros)

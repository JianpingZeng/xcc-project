...
import cfe.cparser.DeclSpec.DeclaratorChunk.ArrayTypeInfo;
import cfe.cparser.DeclSpec.DeclaratorChunk.FunctionTypeInfo;
import cfe.cparser.DeclSpec.DeclaratorChunk.PointerTypeInfo;
import cfe.cparser.DeclSpec.SCS;
import cfe.cparser.DeclSpec.TST;
import cfe.diag.*;
...
import cfe.type.*;
import cfe.type.ArrayType.ArraySizeModifier;
...
import static cfe.ast.CastKind.*;
import static cfe.ast.Tree.ExprObjectKind.OK_BitField;
import static cfe.ast.Tree.ExprObjectKind.OK_Ordinary;
import static cfe.ast.Tree.ExprValueKind.EVK_LValue;
import static cfe.ast.Tree.ExprValueKind.EVK_RValue;
import static cfe.clex.TokenKind.*;
import static cfe.cparser.DeclSpec.TQ.*;
import static cfe.cparser.Declarator.TheContext.FunctionProtoTypeContext;
import static cfe.cparser.Declarator.TheContext.KNRTypeListContext;
import static cfe.cparser.Parser.exprError;
import static cfe.cparser.Parser.stmtError;
import static cfe.sema.AssignConvertType.*;
import static cfe.sema.BinaryOperatorKind.*;
import static cfe.sema.Decl.DefinitionKind.DeclarationOnly;
import static cfe.sema.LookupResult.LookupResultKind.Found;
import static cfe.sema.LookupResult.LookupResultKind.NotFound;
import static cfe.sema.Scope.ScopeFlags.DeclScope;
import static cfe.sema.Sema.AssignAction.*;
import static cfe.sema.Sema.LookupNameKind.*;
import static cfe.sema.UnaryOperatorKind.*;
import static cfe.support.Linkage.ExternalLinkage;
import static cfe.support.Linkage.NoLinkage;
...
public final class Sema implements DiagnosticParseTag,
      ...
      if (literalString != null) {
        ...
        switch (opc) {
          ...
          case BO_NE:
            ...
            break;
          default:
            Util.shouldNotReachHere("Invalid comparison operator");
        }
  ...
  private static void processDeclAttribute(
    ...
    switch (attr.getKind()) {
      case AT_IBOutlet:
        handleIBOutletAttr(decl, attr, sema);
        break;
      case AT_address_space:
      case AT_alias:
        handleAliasAttr(decl, attr, sema);
        break;
      case AT_aligned:
        handleAlignedAttr(decl, attr, sema);
        break;
      case AT_always_inline:
        handleAlwaysInlineAttr(decl, attr, sema);
        break;
      case AT_analyzer_noreturn:
        handleAnalyzerNoReturnAttr(decl, attr, sema);
        break;
      case AT_annotate:
        handleAnnotateAttr(decl, attr, sema);
        break;
      case AT_constructor:
        handleConstructorAttr(decl, attr, sema);
        break;
      case AT_deprecated:
        handleDeprecatedAttr(decl, attr, sema);
        break;
      case AT_destructor:
        handleDestructorAttr(decl, attr, sema);
        break;
      case AT_dllexport:
        handleDLLExportAttr(decl, attr, sema);
        break;
      case AT_dllimport:
        handleDLLImportAttr(decl, attr, sema);
        break;
      case AT_ext_vector_type:
        handleExtVectorTypeAttr(decl, attr, sema);
        break;
      case AT_fastcall:
        handleFastCallAttr(decl, attr, sema);
        break;
      case AT_format:
        handleFormatAttr(decl, attr, sema);
        break;
      case AT_format_arg:
        handleFormatArgAttr(decl, attr, sema);
        break;
      case AT_gnu_inline:
        handleGNUInlineAttr(decl, attr, sema);
        break;
      case AT_mode:
        handleModeAttr(decl, attr, sema);
        break;
      case AT_malloc:
        handleMallocAttr(decl, attr, sema);
        break;
      case AT_nonnull:
        handleNonNullAttr(decl, attr, sema);
        break;
      case AT_noreturn:
        handleNoReturnAttr(decl, attr, sema);
        break;
      case AT_nothrow:
        handleNothrowAttr(decl, attr, sema);
        break;

      case AT_reqd_wg_size:
        handleReqdWorkGroupSize(decl, attr, sema);
        break;

      case AT_packed:
        handlePackedAttr(decl, attr, sema);
        break;
      case AT_section:
        handleSectionAttr(decl, attr, sema);
        break;
      case AT_stdcall:
        handleStdCallAttr(decl, attr, sema);
        break;
      case AT_unavailable:
        handleUnavailableAttr(decl, attr, sema);
        break;
      case AT_unused:
        handleUnusedAttr(decl, attr, sema);
        break;
      case AT_used:
        handleUsedAttr(decl, attr, sema);
        break;
      case AT_vector_size:
        handleVectorSizeAttr(decl, attr, sema);
        break;
      case AT_visibility:
        handleVisibilityAttr(decl, attr, sema);
        break;
      case AT_warn_unused_result:
        handleWarnUnusedResult(decl, attr, sema);
        break;
      case AT_weak:
        handleWeakAttr(decl, attr, sema);
        break;
      case AT_weak_import:
        handleWeakImportAttr(decl, attr, sema);
        break;
      case AT_transparent_union:
        handleTransparentUnionAttr(decl, attr, sema);
        break;
      case AT_blocks:
        handleBlocksAttr(decl, attr, sema);
        break;
      case AT_sentinel:
        handleSentinelAttr(decl, attr, sema);
        break;
      case AT_const:
        handleConstAttr(decl, attr, sema);
        break;
      case AT_pure:
        handlePureAttr(decl, attr, sema);
        break;
      case AT_cleanup:
        handleCleanupAttr(decl, attr, sema);
        break;
      case AT_nodebug:
        handleNodebugAttr(decl, attr, sema);
        break;
      case AT_noinline:
        handleNoinlineAttr(decl, attr, sema);
        break;
      case AT_regparm:
        handleRegparmAttr(decl, attr, sema);
        break;

...
import static cfe.codegen.CodeGenFunction.hasAggregateLLVMType;
import static cfe.type.TypeClass.*;
...
public class CodeGenTypes {
   ...
   */
  private HashMap<cfe.type.Type, backend.type.Type> typeCaches;
   ...
   */
  private HashSet<cfe.type.Type> recordBeingLaidOut;
   ...
   */
  private HashMap<cfe.type.Type, StructType> recordDeclTypes;
  ...
  private HashMap<cfe.type.Type, CGRecordLayout> cgRecordLayout;
  ...
  private HashMap<cfe.type.Type, backend.type.Type> tagDeclTypes;
  private HashMap<cfe.type.Type, backend.type.Type> functionTypes;
   ...
   * @return
   */
  public static TagType verifyFunctionTypeComplete(cfe.type.FunctionType fnType) {
    if (fnType.getResultType().getType() instanceof cfe.type.TagType) {
    ...
    if (fnType instanceof FunctionProtoType) {
      ...
      for (int i = 0; i < fpt.getNumArgs(); i++) {
        if (fpt.getArgType(i).getType() instanceof cfe.type.TagType) {
  ...
  private backend.type.Type convertTagDeclType(Decl.TagDecl td) {
    cfe.type.Type key = context.getTagDeclType(td).getType();
  ...
  private backend.type.Type convertNewType(QualType t) {
    cfe.type.Type ty = context.getCanonicalType(t).getType();
    switch (ty.getTypeClass()) {
      ...
      case FunctionProto: {
        cfe.type.FunctionType fnType = (cfe.type.FunctionType) ty;
      ...
      case Union:
      case Enum: {
        Decl.TagDecl td = ((cfe.type.TagType) ty).getDecl();
  ...
  private backend.type.Type convertRecordDeclType(Decl.RecordDecl decl) {
    cfe.type.Type key = decl.getTypeForDecl();
  ...
  public boolean isSafeToConvert(Decl.RecordDecl rd,
    ...
    cfe.type.Type key = rd.getTypeForDecl();
  ...
  private boolean isSafeToConvert(QualType t, ArrayList<Decl.RecordDecl> alreadyChecked) {
    ...
    if (t.getType() instanceof cfe.type.ArrayType) {
      cfe.type.ArrayType at = (cfe.type.ArrayType) t.getType();
  ...
  private boolean isRecordLayoutComplete(cfe.type.Type rd) {
    return recordDeclTypes.containsKey(rd);
  }

  private boolean isRecordBeingLaidOut(cfe.type.Type ty) {
    return recordBeingLaidOut.contains(ty);
  }

  public boolean noRecordBeingLaidOut() {
    return recordBeingLaidOut.isEmpty();
  }
   ...
   * @return
   */
  private boolean isFuncTypeConvertible(cfe.type.FunctionType fnType) {
  ...
  public CGFunctionInfo getFunctionInfo(Decl.FunctionDecl fd) {
    cfe.type.FunctionType ft = fd.getType().getAsFunctionType();

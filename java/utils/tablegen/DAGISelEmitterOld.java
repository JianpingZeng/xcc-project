...
public class DAGISelEmitterOld extends TableGenBackend {
  private RecordKeeper records;
  private CodeGenDAGPatterns cgp;

  public DAGISelEmitterOld(RecordKeeper rec) {
    records = rec;
    ...
    if (lastMatch != 0) {
      ...
      if (shared.size() == 1) {
        ...
        os.printf("%s// Pattern complexity = %d, cost = %d, size = %d%n",
            ...
            PatternToMatch.getPatternSize(pat.getSrcPattern(), cgp) + addComplexity,
            PatternToMatch.getResultPatternCost(pat.getDstPattern(), cgp),
            PatternToMatch.getResultPatternSize(pat.getDstPattern(), cgp));
      }
  ...
  private void emitInstructionSelector(PrintStream os) {
    ...
    for (Map.Entry<String, ArrayList<PatternToMatch>> itr : patternsByOpcode.entrySet()) {
      ...
      for (Map.Entry<Integer, ArrayList<PatternToMatch>> itr2 : patternsByType.entrySet()) {
        ...
        codeForPatterns.sort(new PatternToMatch.PatternSortingPredicate(cgp));
        ...
        if (opName.equals("ISD.STORE")) {
          PatternToMatch.PatternSortingPredicate cmp = new PatternToMatch.PatternSortingPredicate(cgp);

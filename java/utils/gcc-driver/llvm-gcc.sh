...
if [[ $FLAG_target == "armv7" ]]; then
  GCC="${ARM_GCC}/arm-unknown-linux-gnueabi-gcc -mcpu=cortex-a9"
  GPP="${ARM_GCC}/arm-unknown-linux-gnueabi-g++ -mcpu=cortex-a9"
  GCCCPU="-ccc-host-triple armv7-unknown-linux-gnueabi"
  CPU="-march=arm"
else
  GCC="${CLANG}/clang"
  GPP="${CLANG}/clang++"
...
for src in $srcs; do
  ...
  if [[ $FLAG_linker == "c++" ]]; then
    ...
    echo "can't support fortran!"
    exit 0
  fi

  DCall "${CC} -E ${src} > ${preheader} \
  ${gcc_front_opts}" || Die "GCC front-end compile failed!"

  # Generate LLVM IR file with CLANG
  DCall "${CLANG}/clang ${GCCCPU}                  \
         $( if [[ -n $FLAG_optimization ]];        \
            then echo "-O${FLAG_optimization}";    \
            else echo "-O0"; fi)"                  \
         ${preheader} -emit-llvm -S -o ${llc}
done
...
if [ $compile -ne 0 ] || [ $link -ne 0 ] || [ $assemble -ne 0 ]; then
  ...
  #change for static compilation 2.
    if (( compile )) || (( link )); then
        ...
        DCall "${GCC} ${asm} -c -o ${obj} ${gcc_back_opts}" || Die "GCC back-end compile failed!"
    fi
  done
fi

...
import static jlang.basic.APFloat.CmpResult.*;
import static jlang.basic.APFloat.FltCategory.*;
import static jlang.basic.APFloat.OpStatus.*;
import static jlang.basic.APFloat.RoundingMode.*;
import static jlang.basic.LostFraction.*;
   ...
   when rounding up during hexadecimal output.  */
    ...
    private LostFraction addOrSubtractSignificand(APFloat rhs, boolean subtract)
    {
        ...
        if (subtract)
        {
            ...
            if (reverse)
            {
                carry = tempRhs.subtractSignificand(this, LostFraction != lfExactlyZero?1:0);
                copySignificand(tempRhs);
                sign = !sign;
            }
            else
            {
                carry = subtractSignificand(tempRhs, LostFraction!= lfExactlyZero?1:0);
            }

            if (LostFraction == lfLessThanHalf)
                LostFraction = lfMoreThanHalf;
            else if (LostFraction == lfMoreThanHalf)
   ...
   lost fraction.  */
    ...
    private int normalize(RoundingMode rm, LostFraction LostFraction)
    {
        ...
        if (omsb != 0)
        {
            ...
            if (exponentChange < 0)
            {
                assert LostFraction == lfExactlyZero;
    ...
    private int addOrSubtract(APFloat rhs, RoundingMode rm, boolean subtract)
    {
        ...
        if (fs == opDivByZero)
        {
            ...
            assert category != fcZero || LostFraction == lfExactlyZero;
        }
    ...
    private boolean roundAwayFromZero(RoundingMode rm, LostFraction LostFraction, int bit)
    {
        ...
        switch (rm)
        {
            ...
            case rmNearestTiesToAway:
                return LostFraction == lfExactlyHalf || LostFraction ==  lfMoreThanHalf;
            case rmNearestTiesToEven:
                if (LostFraction == lfMoreThanHalf)
                    return true;

                if (LostFraction == lfExactlyHalf && category != fcZero)
    ...
    private int convertFromDecimalString(String str, RoundingMode rm)
    {
      ...
      */
                ...
                semantics.minExponent - (int) semantics.precision))
        {
            ...
            zeroSignificand();
            fs = normalize(rm, lfLessThanHalf);
        }
        ...
        for (; ; parts *= 2)
        {
            ...
            else
            {
                ...
                powHUerr = (powStatus == opOK
                        && calcLostFraction == lfExactlyZero) ? 0 : 2;
            }
            ...
            HUerr = HUerrBound(calcLostFraction != lfExactlyZero,

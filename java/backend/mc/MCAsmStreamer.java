  ...
  public void emitValueToAlignment(int byteAlignment, long value, int valueSize, int maxBytesToEmit) {
    if (Util.isPowerOf2(byteAlignment)) {
      if (Util.isPowerOf2(byteAlignment)) {
        switch (valueSize) {
          default:
            ...
            break;
          case 1: os.print(mai.getAlignDirective());break;
          case 2: os.print(".p2alignw"); break;
          case 4: os.print(".p2alignl"); break;
          case 8: Util.shouldNotReachHere("Unsupported alignment size!");
        }

        if (mai.getAlignmentIsInBytes())
          os.print(byteAlignment);
        else
          os.print(Util.log2(byteAlignment));

        if (value != 0 || maxBytesToEmit != 0) {
          os.print(", 0x");
          os.printf("0x%x", truncateToSize(value, valueSize));
          if (maxBytesToEmit != 0)
            os.printf(", %d", maxBytesToEmit);
        }
        emitEOL();
        return;
      }
      ...
      switch (valueSize) {
        ...
        case 1: os.print(".balign"); break;
        case 2: os.print(".balignw"); break;
        case 4: os.print(".balignl"); break;
        case 8: Util.shouldNotReachHere("Unsupported alignment size!");
      }

      os.printf(" %d", byteAlignment);
      os.printf(", %d", truncateToSize(value, valueSize));
      if (maxBytesToEmit !=0 )
        os.printf(", %d", maxBytesToEmit);
      emitEOL();
    }
